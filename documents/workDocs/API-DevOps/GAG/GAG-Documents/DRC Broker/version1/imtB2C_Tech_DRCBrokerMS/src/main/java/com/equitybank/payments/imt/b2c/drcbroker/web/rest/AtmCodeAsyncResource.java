...
public class AtmCodeAsyncResource {
    private static final Logger log = LoggerFactory.getLogger(AtmCodeAsyncResource.class);
    ...
    @Autowired
    public AtmCodeAsyncResource(DataPersistenceService dataPersistenceService, AsyncResponseService asyncResponseService, CallBackHTTPService callBackHTTPService, Executor taskExecutor, Gson gson) {
    ...
    @PostMapping(path = "/callback/getatmcode",
            consumes = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE},
            produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    public ResponseEntity<?> receiveAsyncResponse(@RequestBody AtmCodeAsyncFinalResponse asyncResponse) {
        log.info("Received Async Response From PCES Broker Service::: " + gson.toJson(asyncResponse));
        ErrorStatus errorStatus = new ErrorStatus();
        String type = "AsyncGetAtmCode";
        BusinessServiceFinalResponse finalResponse = new BusinessServiceFinalResponse();
        BrokerServiceAtmCodeDTO response = (BrokerServiceAtmCodeDTO) asyncResponseService.processAsyncResponse(asyncResponse, type, errorStatus);
        if (response == null) {
            finalResponse.setRequestID(asyncResponse.getOriginatingTransactionID());
        ...
        type = "GetAtmCodeAsyncSuccess";
        updateTransactionDetailsTable(asyncResponse, response, type, errorStatus);

        //call channel callback
        String url = "";
        type = "AsyncGetAtmCode";
        String jsonRes = response.getBusinessServiceFinalResponse();
