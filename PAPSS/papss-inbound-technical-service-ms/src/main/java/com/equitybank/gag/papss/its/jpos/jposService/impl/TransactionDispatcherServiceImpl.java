...
public class TransactionDispatcherServiceImpl implements TransactionDispatcherService {
    ...
    public TransactionDispatcherServiceImpl(TransactionManagerScalingService tmScalingService,
        ...
        this.space = SpaceFactory.getSpace("tspace:inbound"); // Get shared jPOS space
    }
     ...
     */
    @Override
    public void dispatch(InboundMessageReqObject req) throws CustomException {
        String rrn = req.getMessageSeq();
        String messageType = req.getMessageType();
        String xmlMessageBody = req.getPayload();
        ...
        ctx.put(JposConstants.MESSAGE_TYPE, messageType);
        ctx.put(JposConstants.REQUEST_PAYLOAD_CONTEXT, xmlMessageBody);
        ctx.put(JposConstants.TXN_STATUS, TransactionStatus.PENDING);

        // Set Default flags - to be updated in downstream logic
