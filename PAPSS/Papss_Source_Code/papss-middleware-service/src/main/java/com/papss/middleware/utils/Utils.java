...
public class Utils {
    ...
    public static <T> T toObject(String xml, Class<T> tClass) {
        try {
            ...
            // Fallback for local/anonymous classes that Jackson refuses to deserialize as beans
            try {
                String msg = e.getMessage();
                if (msg != null && msg.contains("local/anonymous")) {
                    // Parse XML to a JsonNode and then populate fields reflectively
                    JsonNode node = xmlMapper.readTree(xml);
                    // Try to create instance using accessible no-arg constructor; if unavailable, use Unsafe
                    T instance;
                    try {
                        java.lang.reflect.Constructor<T> ctor = tClass.getDeclaredConstructor();
                        ctor.setAccessible(true);
                        instance = ctor.newInstance();
                    } catch (NoSuchMethodException nsme) {
                        // No default constructor, allocate without constructor invocation
                        try {
                            Class<?> unsafeClass = Class.forName("sun.misc.Unsafe");
                            java.lang.reflect.Field fUnsafe = unsafeClass.getDeclaredField("theUnsafe");
                            ...
                            T tmp = (T) allocateInstance.invoke(unsafe, tClass);
                            instance = tmp;
                        } catch (Exception ue) {
                            throw nsme; // rethrow to be handled by outer catch
                        }
                    }
                    for (java.lang.reflect.Field f : tClass.getDeclaredFields()) {
                        ...
                        try {
                            if (ft == String.class) {
                                f.set(instance, v.asText());
                            } else if (ft == int.class || ft == Integer.class) {
                                f.set(instance, v.asInt());
                            } else if (ft == long.class || ft == Long.class) {
                                f.set(instance, v.asLong());
                            } else if (ft == boolean.class || ft == Boolean.class) {
                                f.set(instance, v.asBoolean());
                            } else if (ft == double.class || ft == Double.class) {
                                f.set(instance, v.asDouble());
                            }
                        } catch (Exception ignore) {
                            // Skip fields we can't set
                        }
                    }
                    return instance;
                }
            } catch (Exception inner) {
                log.error("Fallback XML deserialization failed for type {}: {}", tClass.getSimpleName(), inner.getMessage(), inner);
            }
    ...
    public static XMLGregorianCalendar generateUtcDateTime() {
        try {
            // Always create calendar in UTC, independent of server timezone
            GregorianCalendar calendar = GregorianCalendar.from(
                    ZonedDateTime.now(ZoneOffset.UTC).withNano(0)
            );
            calendar.setTimeZone(TimeZone.getTimeZone("UTC")); // force UTC explicitly
            ...
            xmlGregorianCalendar.setFractionalSecond(null); // remove fractional seconds
            xmlGregorianCalendar.setTimezone(0);            // force UTC offset (Z)

            return xmlGregorianCalendar;
        } catch (Exception e) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Failed to generate UTC DateTime", e);
        }
    }
