...
function saveDet()
{
	//NIDA Changes Start
	//alert("NIDA on save");
	document.getElementsByName('EntityDocumentBO.PlaceOfIssue')[0].disabled = false;
	document.getElementsByName('Cat_EntityDocumentBO.PlaceOfIssue')[0].disabled = false;
	document.getElementsByName('3_EntityDocumentBO.DocIssueDate')[0].disabled = false;
	document.getElementsByName('cal_EntityDocumentBO.DocIssueDate')[0].disabled = false;
	document.getElementsByName('cal_EntityDocumentBO.DocIssueDate')[0].style.visibility = "visible";
	document.getElementsByName('3_EntityDocumentBO.DocExpiryDate')[0].disabled = false;
	document.getElementsByName('cal_EntityDocumentBO.DocExpiryDate')[0].disabled = false;
	document.getElementsByName('cal_EntityDocumentBO.DocExpiryDate')[0].style.visibility = "visible";
	//NIDA Changes End
    try
    {
        customSave();
...
//changes for callid 460087 ends
	//alert('In Entity_Document::save:: 423');
	...
	var retStat = ValidateFormContents(document.frm2,false);
		//alert('In Entity_Document::save:: 440::retStat::'+retStat);
...
//		return "interactive";
//	}
	//alert('In Entity_Document::save:: 460');
	...
	if(document.getElementsByName('EntityDocumentBO.PlaceOfIssue')[0]!=null)
		...
		//alert('In Entity_Document::save:: 464');
	...
	//alert('In Entity_Document::save:: 478');
...
//alert(indexOfDtOfIssCalender);
	...
	//alert('in js:615:Placeofissue mandatory');
...
//Fix for TOL id:451789 Ends
	...
	if((DocumentType=="Passport Number"||DocumentType=="Driving Licence"||DocumentType=="PAN") && ReferenceNo.length > 25 )
	 ...
	 if(DocumentType=="National Card Number" && ReferenceNo.length > 16 )
	   {
	      showMessage(getUserErrorMessage('MSG076', [MSGJ51517,16]));
	      document.getElementsByName("EntityDocumentBO.ReferenceNumber")[0].select();
	      return "false";
	   }
...
function RefNumValidation()
	{
		
		var ref_num = document.getElementsByName('EntityDocumentBO.ReferenceNumber')[0].value;
		var doc_code =trim(''+document.getElementsByName('hidIdentificationType')[0].value);
		var  DocCode=document.getElementsByName('EntityDocumentBO.DocCode')[0].value ;
		//alert("ref_num.length"+ref_num.length);
		
		if(doc_code=="IDTypeC1")
		{		
			if(ref_num.length!=9)
			{
				alert("Unique ID should be of length 9 for Certificate Of Registration");
				document.getElementsByName('EntityDocumentBO.ReferenceNumber')[0].value = "";
				return interactive;
			}
			
		}
		else if((doc_code=="National Card Number")||(doc_code=="ALIEN CERTIFICATE")||(doc_code=="REFUGEE CERTIFICATE")||DocCode=="DOC03"||DocCode=="ALNID"||DocCode=="REFID")
		{
		
			if(ref_num.length > 16 || ref_num.length < 16)
			{
				alert("Unique ID should be of length 16");
				document.getElementsByName('EntityDocumentBO.ReferenceNumber')[0].value = "";
				return interactive;
			}	
		}			

		var documentNo = document.getElementsByName("EntityDocumentBO.ReferenceNumber")[0].value;
		var strURL = '../http://10.17.6.242:8081/platform-core/smsGateMgw?ref_num = '+documentNo;
		...
		{
			var retval = s_xmlHttp..responseText;
			...
			{
				alert("please give valid unique id");
				document.getElementsByName('EntityDocumentBO.ReferenceNumber')[0].value = "";
				return false;
			}
		}
...
{	var gender;
	...
	var DocumentType =trim(''+document.getElementsByName('hidIdentificationType')[0].value);
	
	 if(DocumentType=="National Card Number" || DocCode=="DOC03" || DocCode=="ALNID" || DocCode=="REFID")
	 
	 {
		 ...
		 //alert("UID"+ref_num);
		var  Unique_gen=ref_num.substring(5,6);
		//alert("Unique_gen"+Unique_gen);
		 
	if(top.window.opener.boName == "AccountBO" || top.window.opener.boName == "AccountModBO")
	{
		
	 gender = top.window.opener.document.getElementsByName("AccountBO.Gender")[0].value;
		//alert("gen"+gender);
		
	}
	//if(Unique_gen=='7' && gender!='F')
		if(Unique_gen != null && Unique_gen != "" && Unique_gen !=undefined){
			if(gender != null && gender != "" && gender !=undefined){
		if(gender=='F' && Unique_gen!='7' )
	{
		alert("Incorrect Gender/UniqueID");
		document.getElementsByName("EntityDocumentBO.ReferenceNumber")[0].value="";
		//top.window.opener.document.getElementsByName("AccountBO.Gender")[0].value="F";
		return interactive;
	}else if(gender=='M' && Unique_gen!='8' )
	{
		alert("Incorrect Gender/UniqueID");
		document.getElementsByName("EntityDocumentBO.ReferenceNumber")[0].value="";
			//top.window.opener.document.getElementsByName("AccountBO.Gender")[0].value="M";
			return interactive;
	}else if (Unique_gen!='8' && Unique_gen!='7'){
		alert("Provide valid UniqueID");
		document.getElementsByName("EntityDocumentBO.ReferenceNumber")[0].value="";
		return interactive;
	}else{
		document.getElementsByName("EntityDocumentBO.ReferenceNumber")[0].value=ref_num;
	}
