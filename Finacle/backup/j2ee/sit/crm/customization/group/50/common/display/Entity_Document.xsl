...
<!-- caching changes end -->
	...
	<head>
		...
		<script type="text/javascript" src="/FinacleCRM/Customization/50/common/js/Entity_Document.js" language="javascript"></script>
...
function NidaLoad() 
{
	//alert("NidaLoad start");
	var docType = getCookie('Filter_docType');
	//alert("docType>>"+docType);
	var docCode = document.getElementsByName('EntityDocumentBO.DocCode')[0].value;
	//alert("docCode>>"+docCode);
	var ReferenceNumber = getCookie('Filter_identNum');
//	alert("ReferenceNumber>>"+ReferenceNumber);
	var placeOfIssue = getCookie('Filter_issueLocation');
	var issueDate = getCookie('Filter_issueDate');
	if(issueDate!="")
	{
		issueDate = convertDate(issueDate);
		//alert("issueDate>>"+issueDate);
	}
	var expiryDate = getCookie('Filter_expiryDate');
	if(expiryDate!="")
	{
		expiryDate = convertDate(expiryDate);
		//alert("expiryDate>>"+expiryDate);
	}
	if(docType=="National Card Number")
	{
		if(docCode=="DOC03")
		{
			if(ReferenceNumber=="")
			{
				...
				document.getElementsByName('3_EntityDocumentBO.DocIssueDate')[0].value = "";
			...
			else
			{
				...
				document.getElementsByName('EntityDocumentBO.ReferenceNumber')[0].disabled = true;
				fnNIDAIssueLocation();
				document.getElementsByName('Cat_EntityDocumentBO.PlaceOfIssue')[0].value = placeOfIssue.toUpperCase();
				document.getElementsByName('Cat_EntityDocumentBO.PlaceOfIssue')[0].disabled = true;
				document.getElementsByName('3_EntityDocumentBO.DocIssueDate')[0].value = issueDate;
				document.getElementsByName('3_EntityDocumentBO.DocIssueDate')[0].disabled = true;
				document.getElementsByName('cal_EntityDocumentBO.DocIssueDate')[0].disabled = true;
				document.getElementsByName('cal_EntityDocumentBO.DocIssueDate')[0].style.visibility = "hidden";
				document.getElementsByName('3_EntityDocumentBO.DocExpiryDate')[0].value = expiryDate;
				document.getElementsByName('3_EntityDocumentBO.DocExpiryDate')[0].disabled = true;
				document.getElementsByName('cal_EntityDocumentBO.DocExpiryDate')[0].disabled = true;
				document.getElementsByName('cal_EntityDocumentBO.DocExpiryDate')[0].style.visibility = "hidden";
			}
		}
		else
		{
	...
	else if(docType=="ALIEN CERTIFICATE")
	{
		if(docCode=="ALNID")
		{
			if(ReferenceNumber=="")
	...
	else if(docType=="REFUGEE CERTIFICATE")
	{
		if(docCode=="REFID")
		{
			if(ReferenceNumber=="")
...
function fnNIDAIssueLocation()
{
	var locale = getCookie('Filter_issueLocation');
	locale = locale.toUpperCase();
	//alert("fnNIDAIssueLocation start");
	var cat_type = "CITY";
	var s_xmlHttp;
	var nida_issueLoc;
	try{
		//alert("fnNIDAIssueLocation try start");
		...
		if(s_xmlHttp.status == 200)
		{
			...
			nida_issueLoc = nodecat_1.text;
			//alert("nida_issueLoc>>"+nida_issueLoc);
		}
		s_xmlHttp.abort();
		if(nida_issueLoc!=null||nida_issueLoc!="")
		{
			document.getElementsByName('EntityDocumentBO.PlaceOfIssue')[0].value = nida_issueLoc;
			document.getElementsByName('EntityDocumentBO.PlaceOfIssue')[0].disabled = true;
		}
		else
		{
	  ...
	  <xsl:choose>
		<xsl:when test="normalize-space($mode)='add' or 'edit'">
			<xsl:call-template name="mdrpdown">
				...
				<xsl:with-param name="onchangefn">loadDocDesc(this);ValidateGender();</xsl:with-param>
			</xsl:call-template>
		</xsl:when>
		<xsl:otherwise>
	...
	<xsl:template name="EntityDocumentBO.ScanRequired">
	 ...
	 <xsl:template name="EntityDocumentBO.ReferenceNumber">
		 <xsl:call-template name="label">
			...
			<xsl:with-param name="fn_onBlur">RefNumValidation();ValidateGender();</xsl:with-param>
			<xsl:with-param name="UsrDataSpace">CustomData</xsl:with-param>
			<xsl:with-param name="starshow">true</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
