...
<xsl:variable name="configFile_4" select="document(concat($file4, ''))"/>
    ...
    <xsl:variable name="file5">../Renderer/Customization/cif/resource/DemographicOtherDetails_<xsl:value-of select="$locale"/>.xml</xsl:variable>
...
<xsl:variable name="configFile_6" select="document(concat($file6, ''))"/>
    <xsl:variable name="configLabels_6" select="$configFile_5/SRMBO/*"/>
...
<xsl:variable name="configFile_7" select="document(concat($file7,''))"/>
    <xsl:variable name="configLabels_7" select="$configFile_7/SRMBO/*"/>
<!-- Alternate BANKING CHANGES Starts-->
