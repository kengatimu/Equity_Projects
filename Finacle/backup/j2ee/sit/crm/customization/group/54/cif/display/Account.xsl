...
function blockCifEdit()
{
	if (operationType == "Edit Entity")
	{
		var cifField = document.getElementsByName("AccountBO.AccountKey")[0].value;

		if (cifField != "" && cifField.length > 0)
		{
			var myCifId = cifField[0].value;
			try
			{
				alert("myCifId: " + myCifId);
			}
			catch(e)
			{
				alert(e.message);
...
<!-- RM codes-->
	<xsl:template name="AccountBO.StrUserField27">
		...
		<xsl:with-param name="selectDesc">StrUserField27</xsl:with-param>
		<xsl:with-param name="selectname">AccountBO.StrUserField27</xsl:with-param>
		<xsl:with-param name="selectdisp">AccountBO.StrUserField27</xsl:with-param>
		<xsl:with-param name="presdata" select="$PresData"/>
		<xsl:with-param name="search_type">value</xsl:with-param>
		<!--<xsl:with-param name="defaultValue">N</xsl:with-param>-->
		<xsl:with-param name="onchangefn">customersocialstatus();</xsl:with-param>
		<xsl:with-param name="choice_name">AccountBO.StrUserField27</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
<!-- Rm codes end-->
