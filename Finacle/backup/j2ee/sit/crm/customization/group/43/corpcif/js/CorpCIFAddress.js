...
/*Tracker 159095 : Sonali  end of changes*/
        ...
        document.frm2.action = '../servlet/com.infy.cis.ui.corpcif.CorpAddressForm_Det?AccessOwnerGroup='+AccessOwnerGroup+'&mode=Edit';
...
//Changes for 442528 end:
		...
		/*Tracker 159095 : Sonali  end of changes*/
		//Equity Bank DRC,Customization changes for CGCR19 start
				document.frm2.ADDR_USERFIELD1.value = srmEscape(ad_Region[i], '^', escCharsAddr);
		//Equity Bank DRC,Customization changes for CGCR19 end
...
// changes for Tracker ID 456637 - TOL ID 455246
			...
			document.frm2.IsAddressProofRcvd.value = srmEscape(ad_IsAddressProofRcvd[i], '^', escChars);
				...
				//Equity bank DRC  CGCR19 changes start
	            document.frm2.action = '../servlet/com.infy.cis.ui.corpcif.CorpAddressForm_Det?IsFormatReadOnly=Yes&mode=Copy';
				//Equity bank DRC  CGCR19 changes end
	           }
		        document.frm2.submit();
            }
...
// changes for Tracker ID 456637 - TOL ID 455246
    ...
    if(!alreadyAdded) {
		...
		//Equity Bank DRC,Customization changes for CGCR19 start
		ad_Region[ad_Region.length]		= region;
		//Equity Bank DRC,Customization changes for CGCR19 start
...
//Changes for CRM10202Beta Ticket 321361 : End
        ...
        TBODY_AdrRecordSet.appendChild(oNewNode);
			...
			//Equity Bank DRC,Customization changes for CGCR19 start
			oNewNode.setAttribute("CorporateBO.Address.userField1", region);
			//Equity Bank DRC,Customization changes for CGCR19 end
			...
			//Equity Bank DRC,Customization changes for CGCR19 start
			oNewNode.setAttribute("CorporateModBO.Address.userField1", region);
			//Equity Bank DRC,Customization changes for CGCR19 start
   ...
   if(window.ActiveXObject){
    oNewNode.attachEvent("onclick", call);
		...
		oNewNode.addEventListener("click",call,false);
	}
	...
	if(boName=='CorporateBO'){
		...
		//Equity Bank DRC,Customization changes for CGCR19 start
		document.all.AdrRecordSet.rows(eduRecordIndex).setAttribute("CorporateBO.Address.userField1",region);
		//Equity Bank DRC,Customization changes for CGCR19 start
		...
		//Equity Bank DRC,Customization changes for CGCR19 start
		document.all.AdrRecordSet.rows(eduRecordIndex).setAttribute("CorporateModBO.Address.userField1",region);
		//Equity Bank DRC,Customization changes for CGCR19 end
