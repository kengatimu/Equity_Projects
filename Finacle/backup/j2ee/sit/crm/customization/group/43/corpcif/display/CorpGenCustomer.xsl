<?xml version="1.0"?> <!--Edit yes default no-->
...
<!-- General header label Fix : -->
    <xsl:variable name="file11">../Renderer/Customization/43/corpcif/resource/CorpGenCustomer_<xsl:value-of select="$locale"/>.xml</xsl:variable>
	
	<!-- Tracker ID :756914 - Chrome Porting Changes -->
    <xsl:variable name="file11XML" select="substring-before(substring-after($file11,'resource/'),'.xml')"/>
<xsl:variable name="configFile_11" select="SRM"/>
			 
     <xsl:variable name="configLabels_11" select="$configFile_11/SRMBO/ResourceFile[@filename=$file11XML]/*"/>	 
...
<!-- Fix : 16 -->
	 ...
	 <xsl:variable name="configFile_3" select="SRM"/>
	 	    ...
	 	    <xsl:variable name="configLabels_6" select="$configFile_3/SRMBO/ResourceFile[@filename=$file3XML]/*"/>
...
<!-- CRM10.2SPE -->
    ...
    <!-- caching changes end -->
     ...
     <xsl:param name="configCoreInter"/>
	  <!-- Tracker ID :756914 - Chrome Porting Changes -->
     <xsl:variable name="configCoreInterXML" select="substring-before(substring-after($configCoreInter,'resource/'),'.xml')"/>
     <xsl:variable name="configFileCore" select="SRM"/>
     <xsl:variable name="configLabelsCore" select="$configFileCore/SRMBO/ResourceFile[@filename=$configCoreInterXML]/*"/>
	 ...
	 <!-- Tracker ID :756914 - Chrome Porting Changes -->
     <xsl:variable name="configFileNameXML" select="substring-before(substring-after($configFileName,'resource/'),'.xml')"/>
     <xsl:variable name="configFile" select="SRM"/>
     <xsl:variable name="configLabels" select="$configFile/SRMBO/ResourceFile[@filename=$configFileNameXML]/*"/>
...
<!-- Changes for KYC Enhancement End-->
   ...
   <!--Changes for call id 406159 recon of 402339 end -->
	...
	<xsl:variable name="phoneCountryCode" select="//SRMData[@SRMDataSpace='Main']/SRMComposite[@n=$boName]/Z[@n=concat($boName,'.phoneCountryCode')]"></xsl:variable>
	<xsl:variable name="phoneCityCode" select="//SRMData[@SRMDataSpace='Main']/SRMComposite[@n=$boName]/Z[@n=concat($boName,'.phoneCityCode')]"></xsl:variable>
	<xsl:variable name="phoneLocalCode" select="//SRMData[@SRMDataSpace='Main']/SRMComposite[@n=$boName]/Z[@n=concat($boName,'.phoneLocalCode')]"></xsl:variable>
...
<!--End of Changes-->
 ...
 <!-- CIF Group Changes Start Naved -->
	...
	var phoneCountryCode='<xsl:value-of select="$phoneCountryCode"/>';
	var phoneCityCode='<xsl:value-of select="$phoneCityCode"/>';
	var phoneLocalCode='<xsl:value-of select="$phoneLocalCode"/>';

function hideStarCorpKeyLabel() {
		try{
		if(isAutoGenKey == 'true') {
			var cifLabel = document.getElementById("lblCorporateBO.corp_Key").innerHTML;
			//Changes for Call id 846436
			//cifLabel = cifLabel.substring(0,cifLabel.indexOf("&lt;strong"));
			cifLabel = cifLabel.substring(0,cifLabel.toLowerCase().indexOf("&lt;strong")); 
			document.getElementById("lblCorporateBO.corp_Key").innerHTML = cifLabel;
		}
		}catch(e){}
	}															   
function functionBodyOnload(){
		if(corporateURL==''){
			hideStarCorpKeyLabel();
			langDesc();
			populateAccContAttr();
			loadID();
			loadGen();
...
function loadSector()
{
	...
	if(operationType =='EditEntity' || operationType =='Create' )
	{
		...
		ISICValidation('SUB_SECTOR_CODE','.sector','.Subsector_Code','SECTOR_CODE');
		CorpISICValue(Acckey1,'Subsector_Code');

		ISICValidation('ISICP1','.Subsector_Code','.StrUserField1','SUB_SECTOR_CODE');
		CorpISICValue(Acckey1,'StrUserField1');
			
		ISICValidation('ISICP2','.StrUserField1','.StrUserField2','ISICP1');		
		CorpISICValue(Acckey1,'StrUserField2');

																					
										  

		ISICValidation('ISIC_DEMAND2','.StrUserField3','.StrUserField4','ISIC_DEMAND1');		
		CorpISICValue(Acckey1,'StrUserField4');

		ISICValidation('SUB_SECTOR_CODE','.StrUserField17','.StrUserField16','SECTOR_CODE')
		CorpISICValue(Acckey1,'StrUserField16');
		
		ISICValidation('ISICP1','.StrUserField16','.StrUserField11','SUB_SECTOR_CODE')
		CorpISICValue(Acckey1,'StrUserField11');
		
		ISICValidation('ISICP2','.StrUserField11','.StrUserField12','ISICP1')
		CorpISICValue(Acckey1,'StrUserField12');

		ISICValidation('ISIC_DEMAND2','.StrUserField13','.StrUserField15','ISIC_DEMAND1')
		CorpISICValue(Acckey1,'StrUserField15');	
...
function loadSegVal(){
 ...
 var Fatca_Reasons=document.getElementsByName("CorporateBO.StrUserField6")[0].value;
 
document.getElementsByName("CorporateBO.StrUserField6")[0].style.visibility="hidden";
...
DebugMessage(e.message);
  ...
  var v=document.getElementsByName(boName+".ForeignAccTaxReportingReq")[0].value;
       if(v=='Y'){
	    document.getElementsByName("CorporateBO.StrUserField1")[0].disabled = false;
	   
       document.getElementsByName("CorporateBO.StrUserField2")[0].disabled = false;
	 
  
	   
       document.getElementsByName("CorporateBO.StrUserField4")[0].disabled = false;
	  ...
	  document.getElementsByName("FATCAReasonsSelected")[0].disabled = false;
	   ...
	   document.getElementsByName("CorporateBO.StrUserField1")[0].value = "";
	   document.getElementsByName("CorporateBO.StrUserField2")[0].value = "";
	   document.getElementsByName("CorporateBO.StrUserField4")[0].value = "";
	  ...
	  //document.getElementsByName("FATCAReasonsSelected")[0].value = "";
	    ...
	    document.getElementsByName("CorporateBO.StrUserField1")[0].disabled = true;
	   
       document.getElementsByName("CorporateBO.StrUserField2")[0].disabled = true;
   
	 
	   
       document.getElementsByName("CorporateBO.StrUserField4")[0].disabled = true;
...
if(corporateURL=='')
{
		...
		screenTitle.titleMsg.innerHTML = "Registered Entity CIF > New Entity > Customer Onboarding";
    ...
    <!-- load() function is renamed to loadGen() -->  
     ...
     <!--tracker id 554568 changes starts-->
      <!--Changes for not auto populating Primary Sol id starts-->
        ...
        <xsl:attribute name="onload">fnLoadFatca();crncyDefaultPop();hideStarCorpKeyLabel();langDesc();populateAccContAttr();loadID();loadGen();makekeyMand();loadKYC();focusFirstField();loadrel();DisableGcifIdLookUp();loadZakat();loadBaseCurrency();isTFPartyFlag();loadBasel();makeSwiftFldMand();storeSegment();showbuttonsVisible(45);swifta();loadAddressType();loadSector();			
...
<!--tracker id 231104 changes ends-->
     ...
     <xsl:when test="normalize-space($IsDummy)='Y'">
        ...
        <!--tracker id 231104 changes starts-->
        <!--tracker id 554568 changes starts-->        
             <xsl:attribute name="onload">fnLoadFatca();fnLoadFatcaReasons();hideStarCorpKeyLabel();langDesc();loadID();loadKYC();loadGen();changeSubSeg();loadrel();DisableGcifIdLookUp();focusFirstField();loadBackEnd();showbuttonsVisible(30);showFormTips();loadSubSeg();loadSegVal();clearDummyAddress('<xsl:value-of select="$IsDummy"/>');fnNullifyDummyCustFlds('<xsl:value-of select="$IsDummy"/>');chkBSN();populateAccContAttr();loadBaseCurrency();makekeyMand();storeSegment();loadCode();makeSwiftFldMand();isTFPartyFlag();loadBasel();swifta();loadSector();
    <!--tracker id 554568 changes ends-->
    <!--tracker id 231104 changes ends-->
    <!-- Tracker ID 131246 CHANGES END BY sonali -->
             </xsl:attribute>
...
<!--ticket id 438296 changes starts-->
	   <!-- Changes for the tracker id 351596 STARTS -->
        <xsl:attribute name="onload">fnLoadFatca();fnLoadFatcaReasons();hideStarCorpKeyLabel();langDesc();loadID();showInReadOnlyMode('<xsl:value-of select="$readMode"/>');loadGen();changeSubSeg();loadSegVal();loadrel();loadKYC();loadBaseCurrency();DisableGcifIdLookUp();populateAccContAttr();swifta();loadBackEnd();chkBSN();isTFPartyFlag();loadBasel();loadCode();makeSwiftFldMand();loadSector();
...
<!-- changes for tracker 336728: removing call to loadSubSeg() -->
 ...
 <!--tracker id 554568 changes starts-->
 <xsl:attribute name="onload">fnLoadFatca();fnLoadFatcaReasons();hideStarCorpKeyLabel();langDesc();loadID();changeSubSeg();loadSegVal();loadGen();onLoadCurrency();loadrel();DisableGcifIdLookUp();loadbutton();focusFirstField();hideFields();loadBackEnd();showbuttonsVisible(30);showFormTips();clearDummyAddress('<xsl:value-of select="$IsDummy"/>');fnNullifyDummyCustFlds('<xsl:value-of select="$IsDummy"/>');chkBSN();populateAccContAttr();loadBaseCurrency();makekeyMand();storeSegment();loadCode();makeSwiftFldMand();isTFPartyFlag();loadBasel();swifta();loadKYC();loadAddressType();loadSector();
<!--tracker id 554568 changes ends-->
...
<!-- ERPBOCF1: ticket id 204295 changes end  -->
 <!-- ticket id 202173 changes end -->
      ...
      <!--  <script type="text/javascript" src="../common/js/PopupMenu.js"></script> -->
        ...
        <script type="text/javascript" language="javascript" src="../Customization/43/corpcif/js/CorpGenCust.js"></script>
	...
	<!-- Equity bank DRC CGCR19 changes start -->
       ...
       <script type="text/javascript" language="javascript" src="../Customization/43/corpcif/js/CorpCIFAddress_RCL.js"></script>
...
function clos(f){
	...
	<!--changes for call id::572933 ends-->
        <!-- Field Rationalization changes end -->
		<xsl:with-param name="fn_onBlur">corpNameTrim();fnDefaultShortName();</xsl:with-param>
...
<!--Equity DRC, Customization changes for PEP Status starts-->
...
<!--Equity DRC, Customization changes for PEP Status ends-->			 
...
<!--isic Secondary activitity changes starts -->
...
<!--isic Secondary activity changes ends -->
...
<!-- Equity bank DRC CGCR-3 changes starts -->  

  <xsl:template name="CorporateBO.StrUserField1">
                 <xsl:call-template name="label">
                 <xsl:with-param name="res_name" select="$configLabels[@name='CorporateBO.StrUserField1']"/>
                   <xsl:with-param name="ele_name">CorporateBO.StrUserField1</xsl:with-param>
                   <xsl:with-param name="ele_val"><xsl:value-of select="$boName"/>.StrUserField1</xsl:with-param>
				   <xsl:with-param name="max_length">100</xsl:with-param>
                 </xsl:call-template>
             </xsl:template>
	 <xsl:template name="CorporateBO.StrUserField2">
                 <xsl:call-template name="label">
                 <xsl:with-param name="res_name" select="$configLabels[@name='CorporateBO.StrUserField2']"/>
                   <xsl:with-param name="ele_name">CorporateBO.StrUserField2</xsl:with-param>
                   <xsl:with-param name="ele_val"><xsl:value-of select="$boName"/>.StrUserField2</xsl:with-param>
				   <xsl:with-param name="max_length">100</xsl:with-param>
                 </xsl:call-template>
             </xsl:template>
	<xsl:template name="CorporateBO.StrUserField4">
                 <xsl:call-template name="label">
                 <xsl:with-param name="res_name" select="$configLabels[@name='CorporateBO.StrUserField4']"/>
                   <xsl:with-param name="ele_name">CorporateBO.StrUserField4</xsl:with-param>
                   <xsl:with-param name="ele_val"><xsl:value-of select="$boName"/>.StrUserField4</xsl:with-param>
				   <xsl:with-param name="max_length">100</xsl:with-param>
                 </xsl:call-template>
             </xsl:template>
			 
	<xsl:template name="CorporateBO.StrUserField6">
                 <xsl:call-template name="label">
                 <xsl:with-param name="res_name" select="$configLabels[@name='CorporateBO.StrUserField61']"/>
                   <xsl:with-param name="ele_name">CorporateBO.StrUserField6</xsl:with-param>
		 ...
		 <xsl:template name="CorporateBO.StrUserField5">
   	<xsl:call-template name="mdrpdown">
   			<xsl:with-param name="res_name" select="$configLabels[@name='CorporateBO.StrUserField5']"/>
			<xsl:with-param name="selectdisp">CorporateBO.StrUserField5</xsl:with-param>
   			<xsl:with-param name="selectname"><xsl:value-of select="$boName"/>.StrUserField5</xsl:with-param>
   			<xsl:with-param name="choice_name">CorporateBO.StrUserField5</xsl:with-param>
   			<xsl:with-param name="search_type">url</xsl:with-param>
        ...
        <xsl:template name="FATCAReasonsSelected">
            <xsl:call-template name="listview">
                ...
                <xsl:with-param name="selectname"><xsl:value-of select="$boName"/>.StrUserField6</xsl:with-param>
        	      <xsl:with-param name="selectdisp">CorporateBO.StrUserField6</xsl:with-param>
                            <xsl:with-param name="search_type">url</xsl:with-param>
                   </xsl:call-template>
        </xsl:template>
...
<!--IDENTIFICATION DOCUMENT CHANGES end-->
			...
			<input type="hidden" name="UserField5" value=""/>
...
<!-- Changes by Ramsurendar end-->
						 ...
						 <table  bgcolor='ffffff' width="100%" border = "0" cellspacing="0" vspace="0" hspace="0" >
						    ...
						    <xsl:call-template name="Begin"/>
						           ...
						           <xsl:call-template name="CorporateBO.ForeignTaxReportingCountry"/>
		                                         ...
		                                         <xsl:call-template name="End"/>
												   <xsl:call-template name="Begin"/>
							 				           <xsl:call-template name="CorporateBO.StrUserField1"/>
							 				          <xsl:call-template name="CorporateBO.StrUserField2"/>
		                                         <xsl:call-template name="End"/>
												   <xsl:call-template name="Begin"/>
							 				           <xsl:call-template name="CorporateBO.StrUserField4"/>	
														<xsl:call-template name="CorporateBO.StrUserField5"/>														   
		                                         <xsl:call-template name="End"/>
												   <xsl:call-template name="Begin"/>
							 				           <xsl:call-template name="CorporateBO.StrUserField6"/>
	...
	<TR >
			...
			<TD  nowrap="true" valign ="top" class="d1" style='width:17%'><xsl:value-of select="$configLabels[@name='CorporateBO.StrUserField6']/@configLabel"/></TD>
			<TD width="10%">
