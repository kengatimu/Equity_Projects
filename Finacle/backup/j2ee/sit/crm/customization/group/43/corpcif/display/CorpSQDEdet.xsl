...
<xsl:variable name="resFile_ML">../Renderer/Customization/43/corpcif/resource/CorpSQDEdet_<xsl:value-of select="$locale"/>.xml</xsl:variable>
<!--Equity DRC, Customization changes for CR20 ends -->
<!-- Tracker ID :756914 - Chrome Porting Changes -->
<xsl:variable name="resFile_MLXML" select="substring-before(substring-after($resFile_ML,'resource/'),'.xml')"/>
<xsl:variable name="resConfigFile" select="SRM"/>
<xsl:variable name="configLabels" select="$resConfigFile/SRMBO/ResourceFile[@filename=$resFile_MLXML]/*"/>
...
<!--End of changes by Niharika for CR356016-->
				       ...
				       <script type="text/javascript" language="javascript" src="../corpcif/js/CorpCIFPhoneEmail.js"></script>
					   	<!-- Equity Bank Customization changes for CGCR19 start-->
				       <script type="text/javascript" language="javascript" src="../Customization/43/corpcif/js/CorpSQDEdet.js"></script>
...
<script>
				<!-- changes for call id 371085 and recon of call id 369315 starts -->
				<!-- Tracker ID :756914 - Chrome Porting Changes -->
				var operaType='<xsl:value-of select="$operationType"/>';
				function functionBodyOnload(){
					if(operaType=='EditEntity'){
						loadseg();
						loadDropDownValues();
...
<xsl:attribute name="onload">custDefaultPhone();loadseg();populateDocTypeOnLoad();onAddrSelect();disableCorpRep();fnload();loadID();loadrel();</xsl:attribute>
</xsl:when>

<xsl:otherwise>
<xsl:attribute name="onload">custDefaultPhone();fnloadCorpSQDE();loadseg();loadprimesolid();populateDocTypeOnLoad();onAddrSelect();disableCorpRep();fnload();loadID();loadrel();</xsl:attribute>
</xsl:otherwise>
</xsl:choose> 
...
//113227 changes ends
		...
		function custDefaultPhone()
{
document.getElementsByName('CorporateBO.PhoneEmail.PhoneEmailType')[0].value="CELLPH";
document.getElementsByName('CorporateBO.PhoneEmail.PhoneEmailType')[0].disabled = true;

	   document.getElementsByName('CorporateBO.PhoneEmail.PhoneNo.cntrycode')[0].value="243";
	   document.getElementsByName('CorporateBO.PhoneEmail.PhoneNo.cntrycode')[0].disabled=true;
       document.getElementsByName("CorporateBO.PhoneEmail.PhoneNo.areacode")[0].disabled =true;
...
<!-- CRM10: Other Maintenence Change by Mamta: Flag, if the field is a NameField: STARTS  -->
				<xsl:with-param name="fn_onBlur">fnDefaultShortName();set_isName(this,true);</xsl:with-param>
...
<!--CGCR16: Country of Birth changes by Bhaswanth M End -->
		...
		<xsl:template name="CorporateBO.PhoneEmail.PhoneEmailType">
			...
			<xsl:with-param name="UsrDataSpace">CustomDat
			
			a</xsl:with-param>
			<xsl:with-param name="onchangefn">fnCountryCodeDefault()</xsl:with-param>
			 <xsl:with-param name="starshow">true</xsl:with-param>
			<xsl:with-param name="presdata" select="$PresData"/>
			</xsl:call-template>
		</xsl:template>
