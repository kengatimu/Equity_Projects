...
function customSave(saveFlag) {
	try{
		if(saveFlag == true){
		//customer_type=document.getElementsByName("AccountBO.Cust_type_code")[0].value;
		emp_type=parent.parent.IFrmtab1.formDispFrame.document.getElementsByName("DemographicModBO.Employment_Status")[0].value;
		doj=parent.parent.IFrmtab1.formDispFrame.document.getElementsByName("3_DemographicBO.MiscellaneousInfo.dtDate1")[0].value;
		
	//if(customer_type == '4' && emp_type == 'Employed')
	if(emp_type == 'Employed')

	{
		if(doj == "" || doj == 'null' || doj == ' null')
		{
			//showMessage("Date of joining is mandatory");
			return false;
		}
	}
	}
...
function customSaveQDE(saveFlag) {
	try{
	//customer_type=document.getElementsByName("AccountBO.Cust_Type")[0].value;
	if(saveFlag == true){
	emp_type=document.getElementsByName("DemographicBO.Employment_Status")[0].value;
	doj=document.getElementsByName("3_DemographicBO.MiscellaneousInfo.dtDate1")[0].value;
	//if(customer_type == '4' && emp_type == 'Employed')
	if(emp_type == 'Employed')
	{
		if(doj == "")
