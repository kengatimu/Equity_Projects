...
#
#   <deploy_tag>  : J2EE-shared | J2EE-<bank_id>     (e.g. J2EE-55)
#   <release_id>  : Azure $(Release.ReleaseId)        (e.g. 487)
#   <deploy_env>  : prod | uat | sit
#
#   Example : suScript.sh 10.0.0.1 J2EE-shared 487 prod
#   Example : suScript.sh 10.0.0.1 J2EE-55     487 uat
#
...
DEPLOY_TAG=$2   # Routing tag from the CD pipeline  e.g. J2EE-shared, J2EE-55
...
if [ -z "$DEPLOY_TAG" ]; then
    error_exit "Missing argument 2: DEPLOY_TAG. Expected: J2EE-shared | J2EE-<bank_id>"
fi
...
#
# Args: $1 deploy_target  (shared | <bank_id>)
...
execute_j2ee_deploy() {
    ...
    log "Running deployment as $FINACLE_APP_USER — target: $deploy_target | env: $deploy_env"
    log "Working directory: $SOURCE_DIR"

    # Heredoc feeds commands via stdin — sudo sees "su - applmgr" with no extra args (matches sudoers).
    # Unescaped vars ($SOURCE_DIR, $deploy_target, $deploy_env) expand now; \$deploy_exit_code evaluates inside applmgr shell.
...
# Extract the deploy target from the tag (e.g. "J2EE-55" → "55", "J2EE-shared" → "shared")
case "$DEPLOY_TAG" in
    "J2EE-"*)
        deploy_target="${DEPLOY_TAG#J2EE-}"
        log "Dispatch — target: $deploy_target | env: $DEPLOY_ENV"
        execute_j2ee_deploy "$deploy_target" "$DEPLOY_ENV"
        ;;
    *)
        error_exit "Unknown deploy tag: '$DEPLOY_TAG'. Expected: J2EE-shared | J2EE-<bank_id>"
        ;;
esac
