<?xml version="1.0"?>
    ...
    <xsl:variable name="file1">../Renderer/Customization/cif/resource/RetCustPhoneEmail_<xsl:value-of select="$locale"/>.xml</xsl:variable>
...
<!--Performance-->
    ...
    <!--Performance-->
	<script type="text/javascript" src="../Customization/cif/js/CIFPhoneEmail.js" language="javascript"></script>
...
/* Code Changes Call Recon of Call 205562 Tracker 250136 Start */
 function enableClose(){
    ...
    /*To disable area code when phone type selected is cellphone*/
    function areaCodeDisable()
    {
     var var1=document.getElementsByName("AccountBO.PhoneEmail.PhoneOrEmail")[0].value;
     if(var1=="Phone");
     {
      var var2= document.getElementsByName("AccountBO.PhoneEmail.PhoneEmailType")[0].value;
      if(var2=='CELLPH')
      {
       document.getElementsByName("AccountBO.PhoneEmail.PhoneNo.areacode")[0].disabled =true;
      }
     }
    }
    /*To disable area code when phone type selected is cellphone*/
...
if(selectedPhoneType=='CELLPH'){
      ...
      var length =areaCode1.length+countryCode1.length+localcode1.length;
         /*To restrict phone length to 12 when phone type selected is cellphone*/ 
         //if(length > 17){
         if(length > 12){
	        alert("The phone number length cannot exceed 12");
	  	return "interactive";
	}
	 /*To restrict phone length to 12 when phone type selected is cellphone*/ 
...
PhoneEmailDetails = new Array();
    ...
    if('<xsl:value-of select="$phoneOrEmail"/>' == 'PHONE' || '<xsl:value-of select="$phoneOrEmail"/>'=="")
    {
	    ...
	    PhoneEmailDetails[1] = trim(''+document.getElementsByName('AccountBO.PhoneEmail.PhoneNo')[0].value);
		               ...
		               <xsl:template name="AccountBO.PhoneEmail.PhoneEmailType">
     			            <xsl:call-template name="mdrpdown">
			                        ...
			                        <xsl:with-param name="search_type">value</xsl:with-param>
	       			                <xsl:with-param name="UsrDataSpace">CustomData</xsl:with-param>
	       			                <!--changes for ticket 220757 and tracker 132940 start-->
			                        <xsl:with-param name="onchangefn">areaCodeDisable();populateLocationCode('Customer');checkForExistingPhoneTypeChk();</xsl:with-param>
			                        <!--changes for ticket 220757 and tracker 132940 ends-->
				         	<xsl:with-param name="presdata" select="$PresData"/>
       			            </xsl:call-template>
		               </xsl:template>
