...
<!-- importing xml files InciDDReq.xml -->
     <xsl:variable name="configFile" select="document(concat('../Renderer/customization/resource/Inci_Insu_Smok_Chg.xml', ''))"/>	
...
<!--template for New Smoker Status -->
...
<xsl:call-template name="EndTabPage"/>
			...
			<xsl:call-template name="BeginTabPage">
					...
					<xsl:with-param name="tabPageDisplayNameRes" select="'Status Details'"/>
					<xsl:with-param name="isTabNameHardCoded" select="'Yes'"/>
					<xsl:with-param name="isInnerToInnerTab" select="'N'"/>
			</xsl:call-template>
