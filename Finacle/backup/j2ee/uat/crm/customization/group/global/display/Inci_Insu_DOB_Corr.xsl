...
<!-- importing xml files InciDDReq.xml -->
     <xsl:variable name="configFile" select="document(concat('../Renderer/customization/resource/Inci_Insu_DOB_Corr.xml', ''))"/>	
...
<!--template for Doc receieved for proof -->
...
<!--template for DOB Day -->
	<xsl:template name="RequestBO.userIntField1">
	<xsl:call-template name="mdrpdown">
	<xsl:with-param name="res_name" select="$configLabels[@name='RequestBO.userIntField1']"/>
	<xsl:with-param name="search_type">value</xsl:with-param>
	<xsl:with-param name="selectdisp">RequestBO.userIntField1</xsl:with-param>
	<xsl:with-param name="selectname">RequestBO.userIntField1</xsl:with-param> 
	</xsl:call-template>
	</xsl:template>

<!--template for DOB Year -->
	<xsl:template name="RequestBO.userIntField2">
	<xsl:call-template name="mdrpdown">
	<xsl:with-param name="res_name" select="$configLabels[@name='RequestBO.userIntField2']"/>
	<xsl:with-param name="search_type">value</xsl:with-param>
	<xsl:with-param name="selectdisp">RequestBO.userIntFieldd2</xsl:with-param>
	<xsl:with-param name="selectname">RequestBO.userIntField2</xsl:with-param> 
	</xsl:call-template>
	</xsl:template>
...
<xsl:call-template name="BeginTabView">
		<xsl:with-param name="tabViewName">tabSRDOBCorr</xsl:with-param>
...
<xsl:call-template name="BeginTabPage">
		<xsl:with-param name="tabViewName">tabSRDOBCorr</xsl:with-param>
		<xsl:with-param name="tabPageName">tpageDOBDetails</xsl:with-param>
		<xsl:with-param name="tabPageDisplayNameRes" select="'DOB Details'"/>
		<xsl:with-param name="isTabNameHardCoded" select="'Yes'"/>
		<xsl:with-param name="isInnerToInnerTab" select="'N'"/>
	</xsl:call-template>
...
<xsl:call-template name="Begin"/>
	<xsl:call-template name="RequestBO.userIntField1"/>
	<xsl:call-template name="RequestBO.userStrField12"/>
<xsl:call-template name="End"/>

<xsl:call-template name="Begin"/>
	<xsl:call-template name="RequestBO.userIntField2"/>
