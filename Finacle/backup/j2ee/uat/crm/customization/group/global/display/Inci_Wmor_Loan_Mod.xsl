...
<!-- importing xml files InciDDReq.xml -->
     <xsl:variable name="configFile" select="document(concat('../Renderer/customization/resource/Inci_Wmor_Loan_Mod.xml', ''))"/>	
...
<!--template for Default Mailing Address1 -->
	...
	<xsl:with-param name="ele_val">RequestBO.userStrField123</xsl:with-param>
	</xsl:call-template>
	</xsl:template>
...
<!--template for Loan Period Months -->
	<xsl:template name="RequestBO.userIntField1">
	<xsl:call-template name="label">
	<xsl:with-param name="res_name" select="$configLabels[@name='RequestBO.userIntField1']"/>
	<xsl:with-param name="ele_name">RequestBO.userIntField1</xsl:with-param>
	<xsl:with-param name="ele_val">RequestBO.userIntField1</xsl:with-param>
	</xsl:call-template>
	</xsl:template>

<!--template for Loan Amount -->
...
<xsl:call-template name="EndTabPage"/>
			...
			<xsl:call-template name="BeginTabPage">
				...
				<xsl:call-template name="BeginTabPage">
					<xsl:with-param name="tabViewName">tabSRAddrChg</xsl:with-param>
					<xsl:with-param name="tabPageName">tpageLoanDetails</xsl:with-param>
					<xsl:with-param name="tabPageDisplayNameRes" select="'Loan Details'"/>
					<xsl:with-param name="isTabNameHardCoded" select="'Yes'"/>
					<xsl:with-param name="isInnerToInnerTab" select="'N'"/>
				</xsl:call-template>
