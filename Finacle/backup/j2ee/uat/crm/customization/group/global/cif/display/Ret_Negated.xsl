...
<!--Changes by Niharika for CR356016-->
    <xsl:variable name="file1">../Renderer/Customization/cif/resource/Ret_Negated_<xsl:value-of select="$locale"/>.xml</xsl:variable>
...
var AttachNotes = '<xsl:value-of select="$AttachNotes"/>';
	     ...
	     <title><xsl:value-of select="//SRMPresentation/SRMSystem[@n='Strings']/SRMString[@n='Title']/@value"/></title>
	      ...
	      <SCRIPT>
			...
			var no_errors;
			//alert('<xsl:value-of select="$operation"/>');
            //366332 changes
		...
		if(chkStat == "true")
		{
					...
					function rejectReasonValue()
					{
						try
						{
							...
							document.getElementsByName("RejectReasonList")[0].Display = "Negated Reasons";
...
<!--
	...
	<xsl:call-template name="Begin"/>
		<td nowrap="true" align="left" valign ="top" class="d1" style="width:181"><xsl:value-of select="$configLabels[@name='AVAILABLE_REJECT_REASON']/@configLabel"/></td>
		<td style="width:1">
         	   <xsl:call-template name="AVAILABLE_REJECT_REASON"/>
                </td>
		<td align="center">
		...
		<td nowrap="true" align="left" valign ="top" class="d1" style="width:175"><xsl:value-of select="$configLabels[@name='REJECT_REASON']/@configLabel"/>&#160;<STRONG class="mandatory_indicator">*</STRONG></td>
		<td>
                   <xsl:call-template name="REJECT_REASON"/>
                </td>
          <xsl:call-template name="End"/>
