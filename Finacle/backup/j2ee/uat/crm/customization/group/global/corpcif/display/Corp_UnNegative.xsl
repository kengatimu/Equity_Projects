<?xml version='1.0'?>
 <xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:import href="../Renderer/common/display/SRMFormDisplay.xsl"/>
    ...
    <xsl:variable name="file1">../Renderer/Customization/corpcif/resource/Corp_UnNegative_<xsl:value-of select="$locale"/>.xml</xsl:variable>
...
var apprFlag='<xsl:value-of select="$apprFlag"/>';//Tracker 96275 Changes for Approval by Ramsurendar
 ...
 // Changes starts for trackerID:235579
		...
		var NoteString = document.frm2.UnNegativeNotes.value;
	    ...
	    for(var i=0;i<z.length;i++)
	    {
	    	ReasonString+=z.options[i].value+sentLimiter;
					...
					function txtLimit1() {
						var x = document.getElementsByName("UnNegativeNotes")[0];
...
<!--template for Note  here-->
  <!--Tracker 78525 Saving without mandatory Fields-->
   <xsl:template name="UnNegativeNotes">
  		   <xsl:call-template name="mtextarea">
  			    <xsl:with-param name="res_name" select="$configLabels[@name='CorporateBO.negative_Notes']"/>
  			    <xsl:with-param name="mvalue">UnNegativeNotes</xsl:with-param>
				...
				<xsl:with-param name="fn_onblur">txtLimit(this,500,"UnNegativeNotes","Un-Negated Notes")</xsl:with-param>
				<!--  Fix for Tracker#79889: Ends -->
  			    <xsl:with-param name="mname">UnNegativeNotes</xsl:with-param>
  			</xsl:call-template>
       </xsl:template>
   <!-- Tracker ID : 59466 BEGIN OF CHANGES -->
	...
	<xsl:call-template name="Begin"/>
			<xsl:call-template name="UnNegativeNotes"/>
