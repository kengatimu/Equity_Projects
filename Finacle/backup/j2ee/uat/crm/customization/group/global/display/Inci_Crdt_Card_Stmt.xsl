...
<!-- importing xml files InciDDReq.xml -->
     <xsl:variable name="configFile" select="document(concat('../Renderer/customization/resource/Inci_Crdt_Card_Stmt.xml', ''))"/>	
...
<!--template for Mode of Statement Delivery -->
...
<!--template for Statement of Month -->
...
<xsl:call-template name="BeginTabView">
		<xsl:with-param name="tabViewName">tabSRCardStmt</xsl:with-param>
