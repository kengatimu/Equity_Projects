<?xml version='1.0'?>
 <xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:import href="../Renderer/common/display/SRMFormDisplay.xsl"/>
    ...
    <xsl:variable name="file1">../Renderer/Customization/cif/resource/Ret_UnBlacklist_<xsl:value-of select="$locale"/>.xml</xsl:variable>
...
<!-- CRA10MSC : TRACKER # 90015 : BEGIN OF CHANGES  --> 
	...
	{ 
		...
		document.frm2.RejectReasonList.Display='Selected UnBlacklisted Reasons'; 
 
	    var chkStat = parent.parent.parent.parent.ValidateFormContents(document.frm2,true);	 
	     
	   // var NoteString = document.getElementsByName("AccountBO.Blacklist_Notes")[0].value; 
	    //var NoteString = document.frm2.WorkNotes.value[];	 
	    var ReasonString=''; 
			...
			ReasonString+=z.options[i].value+sentLimiter; 
					...
					function rejectReasonValue() 
					{
						try 
						{
							...
							document.getElementsByName("RejectReasonList")[0].Display = "UnBlackListed Reasons";
							for (i=0; i &lt; document.frm2.RejectReasonList.length - 1; i++) {
							rejectReason += document.frm2.RejectReasonList.options[i].text + ","; 
							}
...
<!--template for Note  here-->
                <xsl:template name="AccountBO.Blacklist_Notes">
  		   <xsl:call-template name="mtextarea">
  			    <xsl:with-param name="res_name" select="$configLabels[@name='AccountBO.Blacklist_Notes']"/>
				...
				<!--Changes for TOL ID 581262 starts--> 
                                   <xsl:with-param name="fn_onblur">txtLimit(this,500,"AccountBO.Blacklist_Notes","UnBlacklist Notes");checkForEnterKey(this,"AttachNotesUnBlacklist")</xsl:with-param> 
   <!--Changes for TOL ID 581262 ends--> 

				<!--  Fix for Tracker#79889: Ends -->
  			    <xsl:with-param name="mname">AccountBO.Blacklist_Notes</xsl:with-param>
  			</xsl:call-template>
       </xsl:template>
   ...
   <!-- Tracker ID : 59466 BEGIN OF CHANGES -->
     <xsl:template name="AvailableReasons">
	    <xsl:call-template name="listview">
            ...
            <xsl:with-param name="ondblclick">selfield(AvailableReasons, RejectReasonList,false)</xsl:with-param>  
            <!--  CRM10.0ST End -->
    ...
    <xsl:template name="RejectReasonList">
	  ...
	  <xsl:with-param name="IsMandatory">false</xsl:with-param>
            <xsl:with-param name="multiple">false</xsl:with-param>
            <xsl:with-param name="class">listBox</xsl:with-param>
            <xsl:with-param name="ondblclick">selfield(RejectReasonList,AvailableReasons,true)</xsl:with-param>  
	...
	<xsl:call-template name="Begin"/>
	 ...
	 <xsl:call-template name="Begin"/>
	 	...
	 	<TD width="5%" align="center">
	 		...
	 		onclick="selfield(AvailableReasons, RejectReasonList,false)"/><BR/>
	 		<input class='frmBttnsAddRmv' Type='button' value='&lt;&lt;'
	 		onclick="selfield(RejectReasonList,AvailableReasons,true)"/>
	 		</TD>
	 ...
	 <xsl:call-template name="Begin"/>
		...
		<td style='padding-top=10px' align='middle' class="AddRemove">
		   ...
		   <input class="sbttn" Type='button' value=' &gt; ' onclick="selfield(AvailableReasons, RejectReasonList,false)"/><br/><br/>
		</td>  
		<td rowSpan='4' align='middle'  class="d3">
                   <xsl:call-template name="RejectReasonList"/>
                </td>
	  ...
	  <xsl:call-template name="Begin"/>
		   ...
		   <input class="sbttn" Type='button' value=' &lt; ' onclick="selfield(RejectReasonList,AvailableReasons,true)"/>            
		</td>       
	  <xsl:call-template name="End"/>-->			  
