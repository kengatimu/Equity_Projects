...
<xsl:template match="/" >
<xsl:variable name="uniqueID" select="'SaleBO.url_'"/>    
<xsl:variable name="childClass" select="'.SaleCustomer.'"/> 
<xsl:for-each select="SRM">
 <xsl:element name="SRM">
   ...
   <xsl:attribute name="SRMDataSpace"><xsl:value-of select="$attributeNameLocal1"/></xsl:attribute>
	...
	<xsl:element name="SRMComposite">
	<xsl:variable name="attributeNameLocal2" select="@n"/>
       ...
       <xsl:if test="not(contains($select1,'T'))">  
        <xsl:element name="SRMRow"> 
         <xsl:for-each select="Z">                
			...
			<xsl:choose>  
				  <xsl:when test="position()=last()  and not(contains(@n,$childClass)) and not(contains(@n,'SaleContact'))">
				<!--   <xsl:if test="not(contains(@n,url_))">-->
				 ...
				 <!--</xsl:if>-->
				  ...
				  <xsl:when test="position()=last() and (contains(@n,$childClass) or contains(@n,'SaleContact') )">
				    <xsl:call-template name="merge2">
					<xsl:with-param name="position" select="$position" />
					<xsl:with-param name="childClass" select="$childClass" />
				    </xsl:call-template>
 ...
 <xsl:element name="SRMPresentation">
...
<xsl:template name="merge2">
 <xsl:param name="position"/>
	<xsl:param name="childClass"/>
		<xsl:for-each select="/SRM/SRMData/SRMComposite">
		 <xsl:for-each select="SRMRow">
		   <xsl:if test="$position=position()">
		         <xsl:element name="SaleBO.SaleCustomer">	 	  
	 		    <xsl:for-each select="Z">	 			
		...
		<xsl:element name="SaleBO.SaleContact">
		 	  <xsl:for-each select="Z">		 			
		 			<xsl:if test="contains(@n,'SaleContact')">		                               
		 		        <xsl:variable name="element2" select="@n"/>
	 				<xsl:element name="{$element2}">		 							 					<xsl:value-of select="current()"/>
		 				</xsl:element>		 
		 			</xsl:if>		 			
		 		</xsl:for-each>
		 	</xsl:element>	
