...
<!-- importing xml files InciDDReq.xml -->
     <xsl:variable name="configFile" select="document(concat('../Renderer/customization/resource/Inci_Insu_Freq_chg.xml', ''))"/>	
...
<!--template for New Frequency Details -->
...
<xsl:call-template name="BeginTabView">
		<xsl:with-param name="tabViewName">tabSRFreqChg</xsl:with-param>
...
<xsl:call-template name="BeginTabPage">
		<xsl:with-param name="tabViewName">tabSRFreqChg</xsl:with-param>
		<xsl:with-param name="tabPageName">tpageFrequencyDetails</xsl:with-param>
		<xsl:with-param name="tabPageDisplayNameRes" select="'Frequency Details'"/>
		<xsl:with-param name="isTabNameHardCoded" select="'Yes'"/>
		<xsl:with-param name="isInnerToInnerTab" select="'N'"/>
	</xsl:call-template>
