	...
	TabValidator      = item.getAttribute('CorporateBO.TabValidator');
 		...
 		for (var i=0; i <z.length; i++) {
 			...
 			if(z[i].id == "ie5menu1"){
 					document.getElementsByName('ie5menu' + 1)[0].disabled = true;
 					document.getElementsByName('ie5menu' + 2)[0].disabled = false;
 					document.getElementsByName('ie5menu' + 3)[0].disabled = true;
...
// vasanth's changes end
 		...
 		cifprimarysol	= item.getAttribute('CorporateBO.primary_Service_Center');
...
var loggedusersol1=getCookie('loggedusersol');
var cifprimarysol=getCookie('cifprimarysol');
	var temp = new Array();
	temp= loggedusersol1.split(",");
	
	var loggedusersol=temp[0];
	var primarygroupid=temp[1];
	var primarygroupname=temp[2];
	if(primarygroupname == "CIF EDIT APPROVERS")
		{
