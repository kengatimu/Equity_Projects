...
<!-- importing xml files InciDDReq.xml -->
     <xsl:variable name="configFile" select="document(concat('../Renderer/customization/resource/Inci_Core_Rep_DbCard.xml', ''))"/>	
...
<!--		<link rel="stylesheet" type="text/css" href="../Branding/css/common/SRMListTemplate.css"/> -->
	...
	<body  class="popup" bottommargin="0" topmargin="0" style="padding-top=10px; padding-left=5px; padding-right=5px" onload="populateCardNo()" >
...
/*Save Template Function**/
    ...
    {	
		...
		{
			alert("Atleast one Card No should be entered");
		}
	}
...
/*Set one-to-many values Function**/
				...
				function setHiddenvalue()
				{
					...
					for(var i=0; i<5; i ++)
					{
						var cardnum = document.getElementsByName('CardNo')[i].value;
						if (cardnum!="")
						{
							if(addCounter==0)
							{
								
								document.getElementsByName('RequestBO.RequestGenInfo.userStrField1')[0].value=cardnum;
								document.getElementsByName('RequestBO.RequestGenInfo.requestInfoType')[0].value=type;
								addCounter=1;
							}
							else
							{
								document.getElementsByName('RequestBO.RequestGenInfo.userStrField1')[0].value=document.getElementsByName('RequestBO.RequestGenInfo.userStrField1')[0].value+"$"+cardnum;
...
<!--template for Details of Docs sent to Debit Card Center  -->
...
<!--template for Details of Docs recd at Debit Card Center  -->
...
<!--template for Card Despatch Details -->
...
<!--template for Card No -->
	<xsl:template name="CardNo">
...
<xsl:call-template name="BeginTabPage">
	<xsl:with-param name="tabViewName">tabSRAddrChg</xsl:with-param>
	<xsl:with-param name="tabPageName">tpageRepDbCardDetails</xsl:with-param>
...
<xsl:call-template name="BeginTabPage">
	<xsl:with-param name="tabViewName">tabSRAddrChg</xsl:with-param>
	<xsl:with-param name="tabPageName">tpageReplaceDetails</xsl:with-param>
	<xsl:with-param name="tabPageDisplayNameRes" select="'Card Replacement Details'"/>
	<xsl:with-param name="isTabNameHardCoded" select="'Yes'"/>
	<xsl:with-param name="isInnerToInnerTab" select="'N'"/>
</xsl:call-template>

<xsl:call-template name="Begin"/>
	<xsl:call-template name="CardNo">
	<xsl:with-param name="ele_name">CardNo</xsl:with-param>
	</xsl:call-template>
	<xsl:call-template name="CardNo">
	<xsl:with-param name="ele_name">CardNo</xsl:with-param>
	</xsl:call-template>
...
<xsl:call-template name="EndTabView">
	<xsl:with-param name="tabViewName">tabSRAddrChg</xsl:with-param>
	<xsl:with-param name="selectedTabPageName">tpageRepDbCardDetails</xsl:with-param>
</xsl:call-template>
