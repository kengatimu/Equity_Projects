...
<!-- importing xml files InciDDReq.xml -->
     <xsl:variable name="configFile" select="document(concat('../Renderer/customization/resource/Inci_Crdt_Card_Lost.xml', ''))"/>	
...
<!--template for Reason for Blocking -->
...
<xsl:call-template name="BeginTabView">
	<xsl:with-param name="tabViewName">tabSRCardLost</xsl:with-param>
...
<xsl:call-template name="BeginTabPage">
	<xsl:with-param name="tabViewName">tabSRCardLost</xsl:with-param>
	<xsl:with-param name="tabPageName">tpageCardDetails</xsl:with-param>
	<xsl:with-param name="tabPageDisplayNameRes" select="'Card Details'"/>
	<xsl:with-param name="isTabNameHardCoded" select="'Yes'"/>
	<xsl:with-param name="isInnerToInnerTab" select="'N'"/>
</xsl:call-template>
