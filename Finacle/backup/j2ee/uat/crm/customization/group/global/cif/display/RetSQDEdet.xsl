...
<xsl:variable name="resFile_ML">../Renderer/Customization/cif/resource/RetSQDEdet</xsl:variable>
...
<xsl:variable name="Salutation" select="//SRMData[@SRMDataSpace='CustomData']/Z[@n='Salutation']"></xsl:variable>
...
<!--  changes for 314285 start -->
<!-- changes for call id 365117 starts -->
<xsl:variable name="docCode_IDType" select="$customfields[@n='docCode_IDType']"/>
<!-- changes for call id 365117 ends -->
...
<xsl:attribute name="onload"> populateDocTypeOnLoad();fnLoad();populateCountry();</xsl:attribute>
</xsl:when>

<xsl:otherwise>
<xsl:attribute name="onload"> loadprimarysolid();populateDocTypeOnLoad();fnLoad();custFnLoad();</xsl:attribute>
</xsl:otherwise>
</xsl:choose>
<!-- changes done for ticket id 435934 ends -->
...
<xsl:template name="AccountBO.Salutation">
	    ...
	    <xsl:with-param name="presdata" select="$PresData"/>
            ...
            <xsl:with-param name="onchangefn">changeGender()</xsl:with-param>
...
<!--Begin of changes by Bharath for new Gender field -->
<xsl:template name="AccountBO.Gender">
	<xsl:call-template name="mdrpdown">
		...
		<xsl:with-param name="presdata" select="$PresData"/>
                 <xsl:with-param name="defaultValue">F</xsl:with-param>
                <xsl:with-param name="choice_name">AccountBO.Gender</xsl:with-param>
	</xsl:call-template>
</xsl:template>
<!--End of changes by Bharath for new Gender field -->
...
<xsl:template name="AccountBO.Amount2">
	<xsl:call-template name="amountlabel">
		...
		<xsl:with-param name="fncname">rate("2_AccountBO.Amount2","AccountBO.StrUserField13",'<xsl:value-of select="$currencyCode"/>');</xsl:with-param>
