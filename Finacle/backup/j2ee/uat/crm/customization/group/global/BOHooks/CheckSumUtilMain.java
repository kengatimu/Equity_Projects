...
public final class CheckSumUtilMain
{
	// Constructor
	private CheckSumUtilMain()
	{
		// No implementation
	}
								  ...
								  final int aNonCustID) throws Exception
	{
		...
		/* Get the busiObj corresponding to the NonCustomerID received
			   as input param */
		final NonCustomerCheckSumBO boNonCustChk =
		(NonCustomerCheckSumBO) CheckSumHelper.getBOForAttr(
										   aBusiCntxt, "NonCustomerCheckSumBO",
										   NonCustomerCheckSumBO.NONCUSTOMERID,
										   new Integer(aNonCustID));

		if (boNonCustChk == null) { return false;}
		// CheckSum Fields : NoncustomerBO
		...
		// CheckSum Fields : NoncustomerBO... End
		   ...
		   (!(chkSumCalc2.equalsIgnoreCase(tradeChekSum))))
		{
			...
			final String sNonCustBO = "NonCustomerBO";
			final NonCustomerBO boNonCust =
			(NonCustomerBO) CheckSumHelper.getBOForAttr(
											aBusiCntxt, sNonCustBO,
											NonCustomerBO.NONCUSTOMERID,
											new Integer(aNonCustID));
									  ...
									  final int aContactID) throws Exception
	{
		...
		/* Get the busiObj corresponding to the Contact ID received
				   as input param */
		final ContactCheckSumBO boContChk =
		(ContactCheckSumBO) CheckSumHelper.getBOForAttr(
											aBusiCntxt, "ContactCheckSumBO",
											ContactCheckSumBO.CONTACTID,
											new Integer(aContactID));

		if (boContChk == null) { return false;}
		final String[] aString = new String[16];
		// CheckSum Fields : CONTACTBO
		...
		// Input String to CheckSum API : CONTACTBO
		...
		// Input String to CheckSum API : ENTITYRELATIONSHIPBO
		...
		{
			// Call function to write into CheckSumErrorBO
			final String sContactBO = "ContactBO";
			final ContactBO boCont =
			(ContactBO) CheckSumHelper.getBOForAttr(
												aBusiCntxt, sContactBO,
												ContactBO.CONTACTID,
												new Integer(aContactID));
			final String sOrgKey 	= boCont.getContactsKey();
			CheckSumHelper.writeCheckSumError(aBusiCntxt,
												sContactBO, aContactID,sOrgKey);
								  ...
								  final int aAccntID) throws Exception
	{
		...
		/* Get the busiObj corresponding to the Contact ID received
				   as input param */
		final AccountCheckSumBO boAccChk =
		(AccountCheckSumBO) CheckSumHelper.getBOForAttr(
										aBusiCntxt, "AccountCheckSumBO",
										AccountCheckSumBO.ACCOUNTID,
										new Integer(aAccntID));
		if (boAccChk == null) { return false;}
		final String[] aString = new String[16];
		// CheckSum Fields : ACCOUNTBO
		...
		// CheckSum Fields : ENTITYRELATIONSHIPBO
		...
		// CheckSum Fields : ENTITYRELATIONSHIPBO...End
		// CheckSum Fields : TRADEFINANCEBO
		...
		{
			// Call function to write into CheckSumErrorBO
			final String sAccountBO = "AccountBO";
			final AccountBO boAcc =
				(AccountBO) CheckSumHelper.getBOForAttr(
											aBusiCntxt, sAccountBO,
											AccountBO.ACCOUNTID,
											new Integer(aAccntID));
			final String sOrgkey = boAcc.getAccountKey();
			CheckSumHelper.writeCheckSumError(aBusiCntxt,
												sAccountBO, aAccntID,sOrgkey);
									...
									final int aSuspID)throws Exception
	{
		...
		/* Get the busiObj corresponding to the Suspect ID received
					  as input param */
		final SuspectCheckSumBO boSusChk =
				(SuspectCheckSumBO) CheckSumHelper.getBOForAttr(
											aBusiCntxt, "SuspectCheckSumBO",
											SuspectCheckSumBO.SUSPECTID,
											new Integer(aSuspID));
		if (boSusChk == null) { return false;}
	    final String[] aString = new String[16];
		// CheckSum Fields : SUSPECTBO
		...
		{
			// Call function to write into CheckSumErrorBO
			final String sSuspectBO = "SuspectBO";
			final SuspectBO boSus =
				(SuspectBO) CheckSumHelper.getBOForAttr(
												aBusiCntxt, sSuspectBO,
												SuspectBO.SUSPECTID,
												new Integer(aSuspID));
			final String sOrgKey 	= boSus.getSuspectsKey();
			CheckSumHelper.writeCheckSumError(aBusiCntxt,
														sSuspectBO, aSuspID,sOrgKey);
										  ...
										  final int aCorpID)throws Exception
	{
		...
		/* Get the busiObj corresponding to the Corporate ID received
					 as input param */
		final CorporateCheckSumBO boCorpChk =
		(CorporateCheckSumBO) CheckSumHelper.getBOForAttr(aBusiCntxt,
									"CorporateCheckSumBO",
									CorporateCheckSumBO.CORP_ID,
									new Integer(aCorpID));
		if (boCorpChk == null) { return false;}
		final String[] aString = new String[15];
		// CheckSum Fields : CORPORATEBO
		...
		// CheckSum Fields : CORPORATEBO...End
	...
	// Get the checksum already in the table
		...
		{
		// Call function to write into CheckSumErrorBO
		final String sCorporateBO = "CorporateBO";
		final CorporateBO boCorp =
		(CorporateBO) CheckSumHelper.getBOForAttr(aBusiCntxt,
							sCorporateBO,
							CorporateBO.CORP_ID,
							new Integer(aCorpID));
		final String sOrgKey 	  = boCorp.getCorp_Key();
		CheckSumHelper.writeCheckSumError(aBusiCntxt,
												sCorporateBO, aCorpID,sOrgKey);
