...
<!-- importing xml files InciDDReq.xml -->
     <xsl:variable name="configFile" select="document(concat('../Renderer/customization/resource/Inci_Insu_Full_Sur.xml', ''))"/>	
...
/* Function to make A/C number mandatory depending on the value of Mode of Payment */

	function ChangePaymentMode()
	{
		if(document.getElementsByName("RequestBO.userStrField13")[0].value == 'Transfer to Account')
		{

			document.getElementsByName("RequestBO.userStrField15")[0].IsMandatory = "true";
			alert(document.getElementsByName("RequestBO.userStrField15")[0].value);
			
		}
		else if (document.getElementsByName("RequestBO.userStrField13")[0].value == 'Transfer to Policy')
		{
			document.getElementsByName("RequestBO.userStrField16")[0].IsMandatory = "true";
			alert(document.getElementsByName("RequestBO.userStrField16")[0].value);
...
<!--template for Request confirmation details -->
...
<!--template for Mode of Payment -->
	...
	<xsl:with-param name="onchangefn">ChangePaymentMode()</xsl:with-param>
	</xsl:call-template>
	</xsl:template>
...
<!--template for NCB A/C No -->
	<xsl:template name="RequestBO.userStrField15">
	<xsl:call-template name="label">
	<xsl:with-param name="res_name" select="$configLabels[@name='RequestBO.userStrField15']"/>
	<xsl:with-param name="ele_name">RequestBO.userStrField15</xsl:with-param>
	<xsl:with-param name="ele_val">RequestBO.userStrField15</xsl:with-param>
	</xsl:call-template>
	</xsl:template>

<!--template for Transfer Policy Number -->
	<xsl:template name="RequestBO.userStrField16">
	<xsl:call-template name="label">
	<xsl:with-param name="res_name" select="$configLabels[@name='RequestBO.userStrField16']"/>
	<xsl:with-param name="ele_name">RequestBO.userStrField16</xsl:with-param>
	<xsl:with-param name="ele_val">RequestBO.userStrField16</xsl:with-param>
	</xsl:call-template>
	</xsl:template>

<!--template for Cheque number -->
...
<!--template for Collection Details -->
	<xsl:template name="RequestBO.userTextField2">
	<xsl:call-template name="label">
	<xsl:with-param name="res_name" select="$configLabels[@name='RequestBO.userTextField2']"/>
	<xsl:with-param name="ele_name">RequestBO.userTextField2</xsl:with-param>
	<xsl:with-param name="ele_val">RequestBO.userTextField2</xsl:with-param>
	</xsl:call-template>
	</xsl:template>
...
<xsl:call-template name="BeginTabView">
	<xsl:with-param name="tabViewName">tabSRFullSur</xsl:with-param>
...
<xsl:call-template name="BeginTabPage">
	<xsl:with-param name="tabViewName">tabSRFullSur</xsl:with-param>
	<xsl:with-param name="tabPageName">tpageOtherDetails</xsl:with-param>
	<xsl:with-param name="tabPageDisplayNameRes" select="'Other Details'"/>
	<xsl:with-param name="isTabNameHardCoded" select="'Yes'"/>
	<xsl:with-param name="isInnerToInnerTab" select="'N'"/>
</xsl:call-template>
...
<xsl:call-template name="Begin"/>
	<xsl:call-template name="RequestBO.userStrField16"/>
