<?xml version='1.0'?>
 <xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:import href="../Renderer/common/display/SRMFormDisplay.xsl"/>
    ...
    <xsl:variable name="file1">../Renderer/Customization/cif/resource/Ret_UnNegated_<xsl:value-of select="$locale"/>.xml</xsl:variable>
...
<xsl:variable name="configFile_1" select="document(concat($file1, ''))"/>
  ...
  <!-- Recon from CRM10301BSUPP changes starts-->
  ...
  <!-- Recon from CRM10301BSUPP changes ends-->
  <xsl:template match="/">
	<html>
			<head>
<!--Performance-->
...
var apprFlag='<xsl:value-of select="$apprFlag"/>';//CIF Changes for Approval
      var AttachNotes = '<xsl:value-of select="$AttachNotes"/>';
	     ...
	     <title><xsl:value-of select="//SRMPresentation/SRMSystem[@n='Strings']/SRMString[@n='Title']/@value"/></title>
	      ...
	      var aReasons='<xsl:value-of select="$availableReasons"/>'; 
  ...
  <!-- CRA10MSC : TRACKER # 90015 : BEGIN OF CHANGES  --> 
	...
	{ 
		...
		document.frm2.REJECT_REASON.Display='Selected UnNegated Reasons'; 
		...
		{ 
			ReasonString+=z.options[i].value+sentLimiter; 
						...
						if(chkStat == "true") {
							...
							//document.frm2.action = "../servlet/com.infy.cis.ui.cif.NegativeWriter?apprFlag="+apprFlag+"&amp;ReasonString=" + ReasonString + "&amp;NoteString=" + NoteString+ "&amp;FLDHASHKEY=" + FLDHASHKEY + "&amp;SECUREHKEY=" + SECUREHKEY;;
							document.frm2.action = "../servlet/com.infy.cis.ui.cif.NegativeWriter?apprFlag="+apprFlag+"&amp;ReasonString=" + ReasonString + "&amp;FLDHASHKEY=" + FLDHASHKEY + "&amp;SECUREHKEY=" + SECUREHKEY;
					...
					function rejectReasonValue() 
					{
						try 
						{
							...
							document.getElementsByName("REJECT_REASON")[0].Display = "UnNegated Reasons";
					...
					/*Changes by Niharika for CR356016*/
				//window.open('../../../servlet/com.infy.cis.ui.cif.CIFProcessSelectionDetWizard?entityId='+entId+'&amp;entityType='+sentityType+'&amp;isPopup=Yes&amp;SECUREHKEY='+secureHKey+'&amp;FLDHASHKEY='+keyField,'save','directories=No, height=436, left=210, top=120, width=575, location=no, menubar=no, resizable=no, status=no, toolbar=no');
...
<!--template for Note  here-->
   <xsl:template name="AccountBO.Negated_Notes">
  		   <xsl:call-template name="mtextarea">
  			    <xsl:with-param name="res_name" select="$configLabels[@name='AccountBO.Negated_Notes']"/>
				...
				<!--Changes for TOL ID 581262 starts--> 
                                   <xsl:with-param name="fn_onblur">txtLimit(this,500,"AccountBO.Negated_Notes","Suspend Notes");checkForEnterKey(this,"AttachNotesUnNegate")</xsl:with-param> 
   <!--Changes for TOL ID 581262 ends--> 

				<!--  Fix for Tracker#79889: Ends -->
  			    <xsl:with-param name="mname">AccountBO.Negated_Notes</xsl:with-param>
  			</xsl:call-template>
       </xsl:template>
	...
	<xsl:call-template name="Begin"/>
	 ...
	 <xsl:call-template name="Begin"/>
		...
		<td style='padding-top=10px' align='middle' class="AddRemove">
		   ...
		   <input class="sbttn" Type='button' value=' &gt; ' onclick="selfield(AVAILABLE_REJECT_REASON, REJECT_REASON,false)"/><br/><br/>
		</td>  
		<td rowSpan='4' align='middle'  class="d3">
                   <xsl:call-template name="REJECT_REASON"/>
                </td>
	  ...
	  <xsl:call-template name="Begin"/>
		   ...
		   <input class="sbttn" Type='button' value=' &lt; ' onclick="selfield(REJECT_REASON,AVAILABLE_REJECT_REASON,true)"/>            
		</td>       
	  <xsl:call-template name="End"/>	-->		  
