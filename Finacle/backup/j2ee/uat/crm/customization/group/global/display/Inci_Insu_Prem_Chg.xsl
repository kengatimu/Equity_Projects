...
<!-- importing xml files Inci_Insu_Prem_Chg.xml -->
     <xsl:variable name="configFile" select="document(concat('../Renderer/customization/resource/Inci_Insu_Prem_Chg.xml', ''))"/>	
...
<!--template for New Premium Amount -->
...
<xsl:call-template name="BeginTabView">
		<xsl:with-param name="tabViewName">tabSRPremChg</xsl:with-param>
...
<xsl:call-template name="BeginTabPage">
		<xsl:with-param name="tabViewName">tabSRPremChg</xsl:with-param>
		<xsl:with-param name="tabPageName">tpagePremiumDetails</xsl:with-param>
		<xsl:with-param name="tabPageDisplayNameRes" select="'Premium Details'"/>
		<xsl:with-param name="isTabNameHardCoded" select="'Yes'"/>
		<xsl:with-param name="isInnerToInnerTab" select="'N'"/>
	</xsl:call-template>
