...
<!-- importing xml files InciDDReq.xml -->
     <xsl:variable name="configFile" select="document(concat('../Renderer/customization/resource/Inci_Insu_Benr_Chg.xml', ''))"/>	
...
/* Function to change Beneficiary if Update Flag is Y */
	...
	function ChangeBeneficiary()
	{
		...
		for(var i = 0; i < 3; i ++)
		{
			...
			if (nameUpd == "Y")
			{
				...
				document.getElementsByName('DES')[i].IsMandatory = "true";
				document.getElementsByName('Relationship')[i].IsMandatory = "true";
				document.getElementsByName('Percentage')[i].IsMandatory = "true";
...
/*Set one-to-many values Function**/
				...
				function setHiddenvalue()
				{
					...
					for(var i=0; i<6; i ++)
					{
						...
						var relation =document.getElementsByName('RelationShip')[i].value;
						var des =document.getElementsByName('DES')[i].value;
						var perc =document.getElementsByName('Percentage')[i].value;
						...
						if (nameUpd=="Y")
						{
							...
							if(addCounter==0)
							{
								...
								document.getElementsByName('RequestBO.RequestGenInfo.userStrField6')[0].value=relation;
								document.getElementsByName('RequestBO.RequestGenInfo.userStrField7')[0].value=des;
								document.getElementsByName('RequestBO.RequestGenInfo.userIntField1')[0].value=perc;
							...
							else
							{
								...
								document.getElementsByName('RequestBO.RequestGenInfo.userStrField5')[0].value=document.getElementsByName('RequestBO.RequestGenInfo.userStrField6')[0].value+"$"+relation;
								document.getElementsByName('RequestBO.RequestGenInfo.userStrField5')[0].value=document.getElementsByName('RequestBO.RequestGenInfo.userStrField7')[0].value+"$"+des;								
								document.getElementsByName('RequestBO.RequestGenInfo.userStrField5')[0].value=document.getElementsByName('RequestBO.RequestGenInfo.userIntField1')[0].value+"$"+perc;
...
/*Populate A/c No Function**/
		...
		for(var i=0; i<6; i ++)
		{
			...
			document.getElementsByName('RelationShip')[i].value="";
			document.getElementsByName('DES')[i].value="";
			document.getElementsByName('Percentage')[i].value="";
		}
		]]>
		...
		'<xsl:for-each select="//SRMData[@SRMDataSpace='Main']/SRMComposite[@n='RecordSet']/SRMRow">'
			...
			relation	= '<xsl:value-of select="./Z[@n='RequestBO.RequestGenInfo.userStrField6']"/>';
			des			= '<xsl:value-of select="./Z[@n='RequestBO.RequestGenInfo.userStrField7']"/>';
			perc		= '<xsl:value-of select="./Z[@n='RequestBO.RequestGenInfo.userIntField1']"/>';

			<![CDATA[	
			if (type1=="BENEFICIARY")
			{
				alert("B" + iCount);
				...
				document.getElementsByName('RelationShip')[iCount].options.value=relation;
				document.getElementsByName('DES')[iCount].options.value=des;
				document.getElementsByName('Percentage')[iCount].value=perc;
...
<!--template for Update Flag -->
	...
	<xsl:with-param name="onchangefn">ChangeBeneficiary()</xsl:with-param>
	</xsl:call-template>
	</xsl:template>
...
<!--template for DES -->
	<xsl:template name="DES">
	<xsl:param name="selectname"/>
	<xsl:call-template name="mdrpdown">
	<xsl:with-param name="res_name" select="$configLabels[@name='RequestBO.RequestGenInfo.userStrField7']"/>
	<xsl:with-param name="search_type">value</xsl:with-param>
	<xsl:with-param name="selectdisp">RequestBO.RequestGenInfo.userStrField7</xsl:with-param>
	<xsl:with-param name="selectname"><xsl:value-of select ="$selectname"/></xsl:with-param>
	</xsl:call-template>
	</xsl:template>

<!--template for Owner Percentage -->
	<xsl:template name="Percentage">
	<xsl:param name="ele_name"/>
	<xsl:call-template name="label">
	<xsl:with-param name="res_name" select="$configLabels[@name='RequestBO.RequestGenInfo.userIntField1']"/>
	<xsl:with-param name="ele_name"><xsl:value-of select ="$ele_name"/></xsl:with-param>
	<xsl:with-param name="ele_val">RequestBO.RequestGenInfo.userIntField1</xsl:with-param>
	</xsl:call-template>
	</xsl:template>
...
<td>
		...
		<input type="hidden" value="" name="RequestBO.RequestGenInfo.userIntField1"/>
...
<xsl:call-template name="BeginTabPage">
		...
		<xsl:with-param name="tabPageDisplayNameRes" select="'Beneficiary Details'"/>
		<xsl:with-param name="isTabNameHardCoded" select="'Yes'"/>
		<xsl:with-param name="isInnerToInnerTab" select="'N'"/>
</xsl:call-template>
...
<xsl:call-template name="Begin"/>
	...
	<xsl:with-param name="selectname">DES</xsl:with-param>
	</xsl:call-template>				
<xsl:call-template name="End"/>
...
<xsl:call-template name="Begin"/>
	...
	<xsl:with-param name="ele_name">Percentage</xsl:with-param>
	</xsl:call-template>				
<xsl:call-template name="End"/>
