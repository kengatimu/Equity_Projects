...
/*Call ID 433921 : begin of changes*/
	   document.frm2.action = "../servlet/com.infy.cis.ui.common.EntityDocument_Det?mode=edit&choice_nameIDType=ID_TYPE&dual="+dualflag+"&incorporationDate1"+incorporationDate1;
/*Call ID 433921 : end of changes*/
	...
	if(readMode.toUpperCase()=="Y")
	{
		...
		//document.frm2.action = "../servlet/com.infy.cis.ui.common.EntityDocument_Det?readonly=Y&choice_nameIDType=ID_TYPE";
		 document.frm2.action = "../servlet/com.infy.cis.ui.common.EntityDocument_Det?readonly=Y&choice_nameIDType=ID_TYPE&dual="+dualflag;
		  <!--tracker id 113227 ends-->;
	}
...
function save(corpsubmit){
	...
	if(bank_id=="54")
	{
		...
		var highriskcat = document.getElementsByName("CorporateBO.StrUserField12")[0].value;
		var KYC_Date = document.getElementsByName("CorporateBO.KYC_Date")[0].value;
		...
		var KYC_ReviewDate = document.getElementsByName("CorporateBO.KYC_ReviewDate")[0].value;
		...
		if((highriskcat !='')&&( highriskcat!=undefined))
	 	 {
			 if(document.getElementsByName("CorporateBO.riskRating")[0].value == "")
			{			 
				showUserMessage("Please Enter riskRating",'');
				document.getElementsByName("CorporateBO.riskRating")[0].IsMandatory = "true"; 
				return "false";
			}
			if(document.getElementsByName("CorporateBO.submitForKYC")[0].value == "")
			{			 
				showUserMessage("Please Enter submitForKYC",'');
				document.getElementsByName("CorporateBO.submitForKYC")[0].IsMandatory = "true"; 
				return "false";
			}
			if(document.getElementsByName("CorporateBO.KYC_ReviewDate")[0].value == "")
			{
				document.getElementsByName("CorporateBO.KYC_ReviewDate")[0].IsMandatory = "true";
				alert("Please Enter KYC_ReviewDate");
				return "interactive";
			}
			if(document.getElementsByName("CorporateBO.KYC_Date")[0].value == "")
			{
				document.getElementsByName("CorporateBO.KYC_Date")[0].IsMandatory = "true";
			...
			if(document.getElementsByName("CorporateBO.SecondRMLogin_ID")[0].value == "")
			{
				document.getElementsByName("CorporateBO.SecondRMLogin_ID")[0].IsMandatory = "true";
				alert("Please Enter Secondary Relationship Manager ID ");
				return "interactive";
			}
		 }
		 // Changes done by sundar for KYC submit=Y
...
function KRAchecker() {
   ...
   var krapinNumber = document.getElementsByName('CorporateBO.StrUserField8')[0].value;
	if(placeofissue.toUpperCase()=='KENYA'|| placeofissue.toUpperCase()=='KE')
	{
	if (krapinNumber.match(letterNumber) && krapinNumber.length == 11) {
		//alert('Valid KRA!');
		krst='true';}
	else{
		alert(" ENTER A VALID KRA PIN");
	...
	// Tracker ID - 90197 End of Changes for attribute masking
		...
		if(corpsubmit == 'true')
		{
			if(preferredAddress.value == "")
			{
				...
				//Changes for call id: 713374 Start
				parent.parent.parent.parent.messageFrm.addMessage(selTabId, getUserMessage("MSGJ50265",""), preferredAddress.name, 'HYPER_LINK');
				//Changes for call id: 713374 End
				parent.parent.parent.parent.messageFrm.showMessages(selTabId);
				return "interactive";
			}
			else
...
function clearIDValues()
{
	...
	document.frm2.hidEDoc_IsDirty.value ="";//Fix for ticket 712637   	
  }
...
//changes for callid 709300 starts
	if(getCookie('TDSType'))
			    ...
			    var cifID= getCookie('LookupChildCIFID');
							    ...
							    var tempvar = getCookie('corecustId');
