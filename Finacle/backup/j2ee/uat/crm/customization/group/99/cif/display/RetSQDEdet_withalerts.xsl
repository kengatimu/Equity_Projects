...
<xsl:variable name="resFile_ML">../Renderer/Customization/54/cif/resource/RetSQDEdet</xsl:variable>
...
<xsl:attribute name="onload"> alert(9);populateDocTypeOnLoad();alert(1);fnLoad();alert(2);populateCountry();alert(3);</xsl:attribute>
</xsl:when>

<xsl:otherwise>
<xsl:attribute name="onload">alert(10);fnloadSQDE();alert(4);custFnLoad();alert(5);loadprimarysolid();alert(6);populateDocTypeOnLoad();alert(7);fnLoad();alert(8);</xsl:attribute>
</xsl:otherwise>
</xsl:choose>
<!-- changes done for ticket id 435934 ends -->
...
<xsl:template name="AccountBO.Cust_First_Name">
	<xsl:call-template name="label">
		  ...
		  <xsl:with-param name="checkASCII">false</xsl:with-param>
        <xsl:with-param name="fn_onBlur">set_isName(this,true);fnNameLengthSQDE(this);</xsl:with-param>
