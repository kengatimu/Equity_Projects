...
<!--Changes end-->
	...
	<script>
		...
		function blockCifEdit()
		{
			...
			if(myBankId == "54")
			{
				var myCifId = item.getAttribute('CorporateBO.corp_Key');
				if (myCifId != "")
				{
					try
					{
						...
						if (s_xmlHttp.status == 200)
						{
							...
							if (statusValue == "EXISTS")
							{
								...
								var m2 = document.getElementsByName('ie5menu2')[0];
								if (m2)
								{
									m2.onclick = null;
									m2.href = "#";
								...
								var m3 = document.getElementsByName('ie5menu3')[0];
								if (m3)
								{
									m3.onclick = null;
									m3.href = "#";
    ...
    <script type="text/javascript" src="../common/js/PopupMenu.js"></script>
      ...
      <!-- Ticket Id 201963 : Begin
		...
		<xsl:call-template name="mainbody">
			 ...
			 <xsl:with-param name="jscriptfnc">,blockCifEdit();identifyme()</xsl:with-param>
