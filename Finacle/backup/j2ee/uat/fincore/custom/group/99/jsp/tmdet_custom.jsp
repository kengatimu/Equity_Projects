...
{
	//ODSME START
	var Amtcheck = document.forms[0].refAmt.value
	if((obj.id == "refAmt") && (Amtcheck  != ""))
	 {
		ODValidateAmt(Amtcheck);
	}
...
function fnAcctCrncyVal() 
{
	...
	if (errorFlg != "Y") 
	{
	    ...
	    hideImage("sLnk10");
		if (profileId == 50){
                if(!fnIsNull(document.forms[0].rate.value)){
                if(fnIsNull(document.forms[0].tranParticularsCode.value)){
...
function ODOverduecheck()
{
	...
	//var inputNameValues ="acctId|" + acctId
	var inputNameValues ="acctId|" + acctId +"|TranType|" + TranType;
        var outputNames = "";
        var scrName = "RestrictTRan_overdue.scr";
        var retVal = appFnExecuteScript(inputNameValues, outputNames, scrName, false);
        var ret1 = retVal.split("|");
	 if(ret1[1]=="Error")
	 {
	 alert(ret1[0]);
	 document.forms[0].refAmt.disabled = true;
       	 return false;
        } 
}

function ODValidateAmt(refAmt)
{
	//alert("TranType1>>");
	...
	var inputNameValues ="acctId|" + acctId +"|refAmt|" + refAmt +"|TranType|" + TranType;
	//alert(inputNameValues);
        var outputNames = "";
        var scrName = "CheckEffAmt.scr";
        var retVal = appFnExecuteScript(inputNameValues, outputNames, scrName, false);
        var ret1 = retVal.split("|");
	 if(ret1[1]=="Error")
	 {
...
function fnKeyDownProcessor(event){
	...
	switch(getEvtKeyCode(event)){
		...
		default			:
			...
			if(logicalKey == "FORM-SUBMIT")
			{
				 logicalKey = "FORM-POST";
			}
