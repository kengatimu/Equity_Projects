...
<!--     0.2            14-06-2013           Samrat Saha          custom field added
<!---------------------------------------------------------------------------------------------------------------->
...
if(mopId == "HORM")
	{
		...
		{
			//alert(tempRemitCrncy);
			getFieldsFromCustomData("fedWire");
		//bank 364537 start
			var dco = tempRemitCrncy;
				//alert(dco);
				var input      = "dco|"+dco;
				var outputNames    = "bank|outVal";
				var outputNames    = "branch|outVal";
				var scrName     =   "b_ormpartydetails.scr";
				...
				var branch = value;
				//alert(branch);
				document.forms[0].corrBank.value=bank;
				document.forms[0].corrBranch.value=branch;
		//bank 364537 end
...
//---------------------------------------------------------------------
      if((funcCode == 'J')||(funcCode == 'M')){
		...
		var n = remitType.indexOf(subtype2);	
	
		var corrBank = document.forms[0].corrBank.value;
		var corrBranch=document.forms[0].corrBranch;
		corrBranch.onblur = function(){fncorrBranch()};
		/*
		# commented by charles 356550 
		...
		*/
			...
			if(n > 0 ) {
				if(document.forms[0].partyCntry.value==""){
					document.forms[0].partyCntry.value = contrycode;
				}
				
				if(document.forms[0].benefCntry.value==""){
					document.forms[0].benefCntry.value = contrycode;
				}
				return true;
			}
	}
...
function fncorrBranch(){
    ...
    if(n>0){
		...
		var corrBranch=document.forms[0].corrBranch.value;
		var	inputNameValues    = "corrBank" + "|" +corrBank+ "|" + "corrBranch" +"|"+ corrBranch;
             	var	outputNames        = "val1";
		var	scrName            = "rmdfltpopmn005.scr";
		var	retVal             = appFnExecuteScript(inputNameValues, outputNames, scrName, false);
		var ret = retVal.split("|");
		var swiftcode= ret[1];
		var cntryCode= ret[3];
		var cntCode = ret[5];
		document.forms[0].benefName.value = swiftcode;
        if(corrBranch !=""){
			if(cntryCode != cntCode){
				alert("RTGS IS FOR LOCAL BANKS ONLY PLEASE INPUT A LOCAL BANK");
				return false;
			}
        } 
    }		
...
<input type="hidden" class="textfieldfont" id="hiddenremitType" name="hiddenremitType">
