...
<!--     0.2            19-03-2013           Shilpa           split value modified in HORM mop id
<!--     0.3            11-09-2013          Suhasini           Porting 2 changes done
<!---------------------------------------------------------------------------------------------------------------->
...
function post_ONLOAD(obj){
    ...
    if(mopId == "HIRM"){	
		if((funcCode == 'G')||(funcCode == 'M')){
            ...
            //if(sacType == acctType){
				...
				if(drCrFlg == "C" ){ 
					var tempVar ="FrgnRmt"+" "+refCcy+ " " +refAmt + " " +addr1;
                                        if(tempVar.length >30)
                                        {
                                                tempVar  = tempVar.substring(0,30);
                                        }

					//document.forms[0].tranPartcls.value ="FrgnRmt"+" "+refCcy+ " " +refAmt + " " +addr1;
					//document.forms[0].remarks.value ="FrgnRmt"+" "+refCcy+ " " +refAmt + " " +addr1;
					document.forms[0].tranPartcls.value = tempVar;
                                        document.forms[0].remarks.value = tempVar;
					return false;
				}
				else{
					document.forms[0].tranPartcls.value = "COMM ON FOREIGN REMITTANCE";
					document.forms[0].remarks.value = "COMM ON FOREIGN REMITTANCE";
					return false;
				}
			}
		//}
    ...
    if(mopId == "HORM"){
		if((funcCode == 'J')||(funcCode == 'M')){
                ...
                var n = remitType.indexOf(paysisId);
                if(n<0){
				if(drCrFlg == "D" && sacType == acctType ) {
					var tempVar ="FrgnRmt"+" "+refCcy+ " " +refAmt + " " +Name;
					if(tempVar.length >30)
					{
						tempVar  = tempVar.substring(0,30);
					}
					//document.forms[0].tranPartcls.value ="FrgnRmt"+" "+refCcy+ " " +refAmt + " " +Name;
					//document.forms[0].remarks.value ="FrgnRmt"+" "+refCcy+ " " +refAmt + " " +Name;
					document.forms[0].tranPartcls.value = tempVar;
					document.forms[0].remarks.value = tempVar;	
					return true;
				}
			...
			if(n > 0){
				if(drCrFlg == "D" && sacType == acctType){
					var tempVar1 = "RTGS"+" " +refCcy+ " " +refAmt+ "/" +addrBB1;
					if(tempVar1.length >30)
					{
						tempVar1 = tempVar1.substring(0,30);
					}
					//document.forms[0].tranPartcls.value = "RTGS"+" " +refCcy+ " " +refAmt+ "/" +addrBB1;
				        //document.forms[0].remarks.value = "RTGS"+" " +refCcy+ " " +refAmt+ "/" +addrBB1;	
					document.forms[0].tranPartcls.value = tempVar1;
					document.forms[0].remarks.value = tempVar1;
					return true;
				}
				else{
					alert("Visit the customer credit part tran only...");
...
function ormtrandet_pre_TAB_SWITCH()
{
	if((funcCode == 'J')||(funcCode == 'G')||(funcCode == 'C')||(funcCode == 'M'))
	{
        if(document.forms[0].remarks.value == "")
		{
            alert("Customer remarks must be entered.");
...
function ormtrandet_pre_ONCLICK(obj)
{
	if((funcCode == 'J')||(funcCode == 'G')||(funcCode == 'C')||(funcCode == 'M'))
	{
		if(document.forms[0].remarks.value == "")
