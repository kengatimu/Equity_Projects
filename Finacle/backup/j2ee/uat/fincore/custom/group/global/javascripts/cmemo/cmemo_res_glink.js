function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">MEMO PAD MAINTENANCE MENU</td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<input type="button" class="button" id="ok" value="Ok" onClick="javascript:return cmemo_res_ONCLICK1(this);" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cmemo_res_ONCLICK1(obj)
{
	var retVal = "";
	/*
	if (pre_ONCLICK('cmemo_res',obj) == false) { 
		return false;
	}
	*/
	...
	/*
	if (post_ONCLICK('cmemo_res',obj) == false) { 
		return false;
	}
	*/
