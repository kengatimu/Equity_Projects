function printBlock()
{
	writeCustomHeader("cuicsau_crit");
	with (document){
	write('<input type="hidden" name="cuicsau.myIPAddress" id="myIPAddress">');
	...
	write('<a href="javascript:showHelpFile(\'cuicsaucrit_help.htm\');" id="sLnk1">');
	...
	write('<td><select name="' + subGroupName + '.funcCode" id="funcCode" ' + cuicsauProps.get("funcCode_ENABLED") + ' class="textfieldfont">');
...
//	write('<option value="I">' + jspResArr.get("FLT000010") + '</option>');
	...
	write('<input hotKeyId="search1" type="text" name="' + subGroupName + '.acctId" id="acctId" ' + cuicsauProps.get("acctId_ENABLED") + ' size="20" maxlength="20"  class="textfieldfont" onChange="javascript:fnCustValidate();">');
	...
	write('<td class="textfield">');
	write('<input hotKeyId="search1" type="text" name="' + subGroupName + '.instType" id="instType" ' + cuicsauProps.get("instType_ENABLED") + ' size="20" maxlength="20"  class="textfieldfont" onchange="javascript:return cuicsau_crit_ONCHANGE1(this,this);">');
...
//	write('<td class="textfield">');
//	write('<input hotKeyId="search1" type="text" name="' + subGroupName + '.instNum" id="instNum" ' + cuicsauProps.get("instNum_ENABLED") + ' size="20" maxlength="20"  class="textfieldfont" onchange="javascript:return cuicsau_crit_ONCHANGE1(this,this);">');
//	write('</td>');
...
function printFooterBlock()
{
	...
	write('<input type="button" name="Accept" value="Go" class="button" id="Accept" onClick="javascript:return cuicsau_crit_ONCLICK2(this,this);" hotKeyId="Go">');
	write('<input type="button" name="Clear" value="Clear" class="button" id="Clear" onClick="javascript:return cuicsau_crit_ONCLICK3(this);">');
	writeFooter();
	write('</div>');
	}
	} //End with()
...
function cuicsau_crit_ONCHANGE1(obj,p1)
{
	var retVal = "";
	if (pre_ONCHANGE('cuicsau_crit',obj) == false) { 
		return false;
	}
	/*if ((retVal = validateUser(p1)) == false) {
		return false;
	}*/
	if (post_ONCHANGE('cuicsau_crit',obj) == false) { 
		return false;
	}
	return (retVal == undefined) ? true : retVal;
}

function cuicsau_crit_ONCLICK2(obj,p1)
{
	var retVal = "";
...
//        }
	*/
	if (pre_ONCLICK('cuicsau_crit',obj) == false) { 
		return false;
	}
	if ((retVal = fnValAndSubmit(p1)) == false) {
		return false;
	}
	if (post_ONCLICK('cuicsau_crit',obj) == false) { 
		return false;
	}
	return (retVal == undefined) ? true : retVal;
}

function cuicsau_crit_ONCLICK3(obj)
{
	var retVal = "";
	if (pre_ONCLICK('cuicsau_crit',obj) == false) { 
		return false;
