function printBlock()
{
	writeCustomHeader("htlrdena_res");
	with (document){
...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="Ok"	onClick="javascript:return htlrdena_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function htlrdena_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('htlrdena_res',obj,'ONCLICK') == false) {
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('htlrdena_res',obj,'ONCLICK') == false) {
		return false;
	}
