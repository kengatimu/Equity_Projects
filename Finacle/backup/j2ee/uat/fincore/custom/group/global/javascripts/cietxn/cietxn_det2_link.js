function cietxn_det2_post_ONLOAD()
{
	...
	if(funcCode == "A"){
	   ...
	   if(ObjForm.disablePrintFlg.value=="Y"){
...
function fnGetEntityDesc(){
        ...
        if(fnIsNull(targetEntity)){
				...
				if(ObjForm.targetEntity.value == "43")
				{
				alert("Inter Country Cash Withdrawal Blocked for DRC Entity");
...
{
	alert("Debit Entity Id should be entered first");
...
function fnGetAcctDetails(){
	...
	fnEnableAcctDetails();
	var drAcctNum = ObjForm.drAcctNum.value;
	var inputNameValues = "acctNum|"+drAcctNum+"|targetEntity|"+ObjForm.targetEntity.value;
...
function fnComputeTranAmt(obj){
	...
	ObjForm.treaRate.disabled = true;
        ...
        var inputNameValues = "drAcctNum|"+drAcctNum+"|drAcctNumEntityId|"+drAcctNumEntityId+"|refCrncy|"+refCrncy+"|acctCrncy|"+acctCrncy+"|refAmt|"+refAmt;
        ...
        var scriptName = "cietxndp056.scr";
        var retVal = appFnExecuteScript(inputNameValues,outputNames,scriptName,false);
        var token = retVal.split("|");
        if(token != undefined){
                var errorFlg = token[1];
...
var ObjForm = document.forms[0];
        if(fnValidateData()){
                ...
                var retVal ;
                if(btnObj.id=="Submit"){
                        var returnValue1 = ObjForm.returnValue1.value;
                        if(fnIsNull(returnValue1)){
...
//Added for biometric validation - Dileep
function doLocalBioMetricValOthBox()
{
         ...
         **************************************************************************************/
                ...
                if(retVal4Menu == true)
                {
                        getFieldforBioAndCallBioOthBox(mopId,document.forms[0].drAcctNumEntityId.value);
                }
        return true;
}
...
function fnPrintReceipt(){
		...
		//if (document.forms[0].targetEntity.value == "11")
		//{
			//alert("Cannot withdraw from Sudan");
			//document.forms[0].targetEntity.focus();
			//return false;
		//}
if(fnValidateData()){
        if(fnIsNull(rptFilePath)){
                var frm = document.forms[0];
                var inputNameValues="entityId|"+frm.targetEntity.value+"|drAcNum|"+frm.drAcctNum.value+"|refCrncy|"+frm.refCrncy.value+"|refAmt|"+removeCommas(frm.refAmt.value)+"|acctName|"+frm.drAcctName.value;
