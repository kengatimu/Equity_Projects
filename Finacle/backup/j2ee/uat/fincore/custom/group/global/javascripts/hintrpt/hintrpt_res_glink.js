...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT000556") + '"	onClick="javascript:return hintrpt_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function hintrpt_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('hintrpt_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('hintrpt_res',obj,'ONCLICK') == false) { 
		return false;
	}
