...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT031310") + '"	onClick="javascript:return clrpt_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function clrpt_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('clrpt_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('clrpt_res',obj,'ONCLICK') == false) { 
		return false;
	}
