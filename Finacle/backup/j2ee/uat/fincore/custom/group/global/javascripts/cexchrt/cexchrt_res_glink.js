...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT030541") + '"	onClick="javascript:return cexchrt_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cexchrt_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cexchrt_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cexchrt_res',obj,'ONCLICK') == false) { 
		return false;
	}
