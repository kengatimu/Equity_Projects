function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">POLARIS Charges Maintenance </td>');
	write('</tr>');
	write('</table>');
	...
	write('<tr>');
	write('<a href="javascript:showHelpFile(\'polaris_det_help.htm\');" id="sLnk1">');
	...
	write('<td><select name="' + subGroupName + '.funcCode" id="funcCode" ' + polarisProps.get("funcCode_ENABLED") + ' class="textfieldfont" onchange="javascript:f1()">');
	...
	write('<option value="Y" selected>ANNUAL SUBSCRIPTION FEE</option>');
	...
	*/
	...
	write('<tr>');
	write('<td class="textlabel">Corporate Account<font color=red size=2>*</font></td>');
	write('<td><input name="' + subGroupName + '.foracid" id="foracid" ' + polarisProps.get("foracid_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" onBlur="javascript:return polaris_ONBLUR12(this);"  onchange="javascript:return polaris_ONCHANGE1(this,this,\'acctName\',\'acctSolId\',\'acctCrncy\',true,\'VALCUSTACCTID\');">');
	...
	/*Text Field for Single Entry*/	 
	write('<tr>');
	write('<td class="textlabel">Charge Account <font color=red size=2>*</font></td>');
	write('<td><input name="' + subGroupName + '.Chargeforacid" id="Chargeforacid" ' + polarisProps.get("Chargeforacid_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" onBlur="javascript:valChargeforacid();">');
	/*write('<td><input name="' + subGroupName + '.Chargeforacid" id="Chargeforacid" ' + polarisProps.get("Chargeforacid_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" onBlur="javascript:return polaris_ONBLUR12(this);"  onchange="javascript:return polaris_ONCHANGE1(this,this,\'acctName\',\'acctSolId\',\'acctCrncy\',true,\'VALCUSTACCTID\');">');
	 write('<a id="sLnk2" target=_self href="javascript:fnshowAccountIdList(document.forms[0].Chargeforacid);" >');
	write('<img id="benfList1" width="16" height="17" border="0"  hotKeyId="search1" src="../Renderer/images/search_icon.gif"></img></a>');
	write('<input type="text" name="acctCrncy" id="acctCrncy" size="3" maxlength="3"  class="label" readonly="readonly" disabled>');
	write('<input type="text" name="acctSolId" id="acctSolId" size="8" maxlength="8"  class="label" readonly="readonly" disabled>');
	write('<input type="text" name="acctName" id="acctName"  size="50" maxlength="50" class="label" readonly="readonly" disabled>');
	*/
	...
	/*
	write('<tr>');
	write('<td class="textlabel">"Charge Account"</td>');
	write('<td><input name="' + subGroupName + '.employer" id="employer" ' + polarisProps.get("employer_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" >');
	...
	*/
	...
	write('<tr>');
	write('<td class="textlabel">Total Charge Amount<font color=red size=2>*</font></td>');
	write('<td><input name="' + subGroupName + '.amount" id="amount" ' + polarisProps.get("amount_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" onBlur="javascript:valAmount();">');
	write('</td>');
	write('<td>&nbsp;</td>');
	write('<td>&nbsp;</td>');
	write('<td>&nbsp;</td>');
	...
	write('<tr>');
	write('<td class="textlabel">Corporate Charges Percentage<font color=red size=2>*</font></td>');
	write('<td><input name="' + subGroupName + '.score" id="score" ' + polarisProps.get("score_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" onBlur="javascript:valScore();">');
	write('</td>');
	write('<td>&nbsp;</td>');
	write('<td>&nbsp;</td>');
	write('<td>&nbsp;</td>');
	...
	write('<tr>');
	write('<td class="textlabel">Customer Charges Percentage<font color=red size=2>*</font></td>');
	write('<td><input name="' + subGroupName + '.period" id="period" ' + polarisProps.get("period_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" onBlur="javascript:valPeriod();">');
	write('</td>');
	write('<td>&nbsp;</td>');
	write('<td>&nbsp;</td>');
	write('<td>&nbsp;</td>');
...
/*
	...
	write('<td class="textfield"><input type="file" name="' + subGroupName + '.fileName" id="fileName" ' + polarisProps.get("fileName_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="100" /></td>');
	write('<td>&nbsp;</td>');
	write('<td>&nbsp;</td>');
	write('<td>&nbsp;</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<div class="ctable">');
        write('<input name="Submit" type="button" class="button" onClick="javascript:return polaris_det_ONCLICK1(this,this);" id="Submit" value="Submit" hotKeyId="Submit">');
	write('<input name="Clear" type="button" class="button" onClick="javascript:return polaris_det_ONCLICK2(this);" id="Clear" value="Clear">');
	}else{
...
//			return false;
  //  	}
	if(document.forms[0].funcCode.value == 'A' || document.forms[0].funcCode.value == 'Y')
		{
			...
			if (document.forms[0].funcCode.value == 'Y')
			{
				document.forms[0].score.value="100"
				//alert(document.forms[0].score.value);
				document.forms[0].period.value="0"
				//alert(document.forms[0].period.value);
			}
...
function polaris_det_ONCLICK2(obj)
{
	var retVal = "";
	/*if (pre_ONCLICK('polaris_det',obj) == false) { 
		return false;
	}*/
	if ((retVal = fnClearFields()) == false) {
		return false;
	}
	/*if (post_ONCLICK('polaris_det',obj) == false) { 
		return false;
	}*/
	return (retVal == undefined) ? true : retVal;
}
...
function polaris_ONBLUR12(obj)
{
	if(document.forms[0].funcCode.value == 'M' || document.forms[0].funcCode.value == 'V' || document.forms[0].funcCode.value == 'Z' || document.forms[0].funcCode.value == 'I' || document.forms[0].funcCode.value == 'Y')
	{
...
function autopopulate(obj)
{
	...
	if(funcCode != 'A' && funcCode != 'Y')
	{
		var inputNameValues = "foracid|"+foracid + "|" + "funcCode|" + document.forms[0].funcCode.value;
		var outputNames     = "";
		if (funcCode == 'V')
		{
			var scrName       = "polarismn004.scr";
		}
		if (funcCode != 'V')
		{
			var scrName         = "polarismn003.scr";
		}		
		...
		//alert(ret[7]);
		if(ret[0] =="EMPLOYER"){
			document.getElementById("Chargeforacid").value = ret[1];
			document.getElementById("amount").value = ret[3];
			document.getElementById("period").value = ret[5];
			document.getElementById("score").value = ret[7];
			//document.getElementById("Chargeforacid").disabled =true;
		}
		return true;
	}
}
...
function fnCustEmployerSearcher(){
       ...
       var outputNames = "employer|empName";
        var scrName     = "catodscoredp006.scr";
...
function polaris_ONCHANGE1(obj,p1,p2,p3,p4,p5,p6)
{
...
function fnFnCodeValidate(){
		...
		var outputNames     = "";
		var scrName         = "catodscoredp007.scr";
		var retVal          = appFnExecuteScript(inputNameValues, outputNames, scrName, false);
