function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">Cheque Book Request Maintenance</td>');
	write('</tr>');
	write('</table>');
	...
	write('<tr>');
	write('<a href="javascript:showHelpFile(\'cchqd_det_help.htm\');" id="sLnk1">');
	...
	write('<td><input name="' + subGroupName + '.duplicate" id="duplicate" ' + cchqdProps.get("duplicate_ENABLED") + ' type="radio" value = "Y" ONCLICK="javascript:fnOnclickRadio()" >Yes<input name="' + subGroupName +'.duplicate" id="duplicate" ' + cchqdProps.get("duplicate_ENABLED") + ' type="radio" value = "N" ONCLICK="javascript:fnOnclickRadio()">No');
    write('</td>');
    write('</tr>');
    ...
    write('<td><input name="' + subGroupName + '.dnldID" id="dnldID" ' + cchqdProps.get("dnldID_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" >');
...
function printFooterBlock()
{
	...
	write('<div class="ctable">');
        write('<input name="Submit" type="button" class="button" onClick="javascript:return cchqd_det_ONCLICK1(this,this);" id="Submit" value="Submit" hotKeyId="Submit">');
        write('<input name="Clear" type="button" class="button" onClick="javascript:return cchqd_det_ONCLICK2(this);" id="Clear" value="Clear">');
...
function fnOnLoad()
{
	...
	ObjForm.frmdate_ui.focus();
	pre_ONLOAD('cchqd_det',this);
...
function cchqd_det_ONCLICK1(obj,p1)
{
/*	var retVal = "";
	if (pre_ONCLICK('cchqd_det',obj) == false) { 
		return false;
	}
*/
	...
	var frm = document.forms[0];
       ...
       frm.submit();
	*/
	if (post_ONCLICK('cchqd_det',obj) == false) { 
	return false;
	}
	return (retVal == undefined) ? true : retVal;
}

function cchqd_det_ONCLICK2(obj)
{
	var retVal = "";
/*
	if (pre_ONCLICK('cchqd_det',obj) == false) { 
