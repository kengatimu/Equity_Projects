function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">' + jspResArr.get("FLT032270") + '</td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT031310") + '"	onClick="javascript:return pklink_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function pklink_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('pklink_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('pklink_res',obj,'ONCLICK') == false) { 
		return false;
	}
