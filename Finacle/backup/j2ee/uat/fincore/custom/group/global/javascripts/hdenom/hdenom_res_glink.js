...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT700008") + '"	onClick="javascript:return hdenom_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function hdenom_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('hdenom_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('hdenom_res',obj,'ONCLICK') == false) { 
		return false;
	}
