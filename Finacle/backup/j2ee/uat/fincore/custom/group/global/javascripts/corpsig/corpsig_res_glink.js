...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT030311") + '"	onClick="javascript:return corpsig_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function corpsig_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('corpsig_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('corpsig_res',obj,'ONCLICK') == false) { 
		return false;
	}
