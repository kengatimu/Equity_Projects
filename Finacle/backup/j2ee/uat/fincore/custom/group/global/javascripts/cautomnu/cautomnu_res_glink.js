function printBlock()
{
	writeCustomHeader("cautomnu_res");
	with (document){
	...
	write('<tr>');
	//write('<td class="page-heading">' + jspResArr.get("FLT031870") + '</td>');
...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT700124") + '"	onClick="javascript:return cautomnu_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cautomnu_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cautomnu_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cautomnu_res',obj,'ONCLICK') == false) { 
		return false;
	}
