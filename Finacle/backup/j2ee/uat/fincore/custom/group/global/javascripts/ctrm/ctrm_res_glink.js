...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT700008") + '"	onClick="javascript:return ctrm_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function ctrm_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('ctrm_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('ctrm_res',obj,'ONCLICK') == false) { 
		return false;
	}
