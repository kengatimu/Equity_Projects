...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT648010") + '"	onClick="javascript:return cagtm_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cagtm_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cagtm_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cagtm_res',obj,'ONCLICK') == false) { 
		return false;
	}
