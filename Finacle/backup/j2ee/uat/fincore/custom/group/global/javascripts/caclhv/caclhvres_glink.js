...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT648069") + '"	onClick="javascript:return caclhvres_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function caclhvres_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('caclhvres',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('caclhvres',obj,'ONCLICK') == false) { 
		return false;
	}
