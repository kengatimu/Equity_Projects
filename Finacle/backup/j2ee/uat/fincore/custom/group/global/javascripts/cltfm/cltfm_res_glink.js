...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT030541") + '"	onClick="javascript:return cltfm_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cltfm_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cltfm_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cltfm_res',obj,'ONCLICK') == false) { 
		return false;
	}
