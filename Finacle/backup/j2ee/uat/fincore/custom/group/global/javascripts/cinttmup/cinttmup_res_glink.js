function printBlock()
{
	writeCustomHeader("cinttmup_res");
	with (document){
	...
	write('<tr>');
	write('<td class="page-heading">BULK LOAN ACCOUNT INTEREST DETAILS UPLOAD</td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<div class="ctable">');
        write('<input type="button" class="button" id="ok" value="Ok" onClick="javascript:return cinttmup_res_ONCLICK1(this);" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cinttmup_res_ONCLICK1(obj)
{
	var retVal = "";
	/*
	if (pre_ONCLICK('cinttmup_res',obj) == false) { 
		return false;
	}
	*/
	...
	/*
	if (post_ONCLICK('cinttmup_res',obj) == false) { 
		return false;
	}
	*/
