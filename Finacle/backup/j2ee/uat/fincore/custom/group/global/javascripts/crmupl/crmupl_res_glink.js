function printBlock()
{
	...
	write('</tr>');
	write('<tr>');
	write('<td class="page-heading">Relationship Manager Maintenance</td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<input type="button" class="button" id="ok" value="Ok" onClick="javascript:return crmupl_res_ONCLICK1(this);" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function crmupl_res_ONCLICK1(obj)
{
	var retVal = "";
	/*
	if (pre_ONCLICK('crmupl_res',obj) == false) { 
		return false;
	}
	*/
	...
	/*
	if (post_ONCLICK('crmupl_res',obj) == false) { 
		return false;
	}
	*/
