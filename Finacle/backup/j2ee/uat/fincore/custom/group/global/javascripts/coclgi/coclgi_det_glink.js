function printBlock()
{
	...
	if(bankId == "56"){
	write('<tr>');
	write('<td class="page-heading">Clearing File </td>');
	write('</tr>');
	}
	else{
	write('<tr>');
	...
	write('<tr>');
	write('<a href="javascript:showHelpFile(\'coclgi_det_help.htm\');" id="sLnk1">');
	...
	write('</tr>');
        ...
        write('<td><input name="' + subGroupName + '.printp" id="printp" ' + coclgiProps.get("printp_ENABLED") + ' type="text" class="textfieldfont" size="10" maxlength="10" disabled="true">');  
...
//	write('<td><input name="' + subGroupName + '.funcCode" id="funcCode" ' + coclgiProps.get("funcCode_ENABLED") + ' class="textfieldfont" onchange="javascript:fnOnchange() ">');
 //       write('</select>');
//       write('</td>');
//        write('</tr>');
	...
	write('<td><input name="' + subGroupName + '.zoneCode" id="zoneCode" ' + coclgiProps.get("zoneCode_ENABLED") + ' type="text" class="textfieldfont" size="10" maxlength="10" >');
	...
	write('<td><input type="text" class="textfieldfont" id="date_ui"" name="' + subGroupName + '.date_ui" fdt="uidate" mnebl="false" fmnd="Y"  fblk="defaultFblk1" hotKeyId="calender1" onChange = "javascript:validateDateOnBlur(this,this,this);">');
...
function printFooterBlock()
{
	...
	write('<div class="ctable">');
        write('<input name="Submit" type="button" class="button" onClick="javascript:return coclgi_det_ONCLICK1(this,this);" id="Submit" value="Submit" hotKeyId="Submit">');
        write('<input name="Clear" type="button" class="button" onClick="javascript:return coclgi_det_ONCLICK2(this);" id="Clear" value="Clear">');
...
function coclgi_det_ONCLICK1(obj,p1)
{
	...
	/*if (pre_ONCLICK('coclgi_det',obj) == false) { 
		return false;
	}*/
	if ((retVal = fnValAndSubmit(p1)) == false) {
		return false;
	}
	if (post_ONCLICK('coclgi_det',obj) == false) { 
		return false;
	}
...
function coclgi_det_ONCLICK2(obj)
{
	var retVal = "";
	/*
	if (pre_ONCLICK('coclgi_det',obj) == false) { 
...
function openDate(obj,inpDate)
{
        ...
        date = popCalendarModalWindowVar("../arjspmorph/"+applangcode+"/date.jsp?txtDate="+obj.id+"&date="+inpDate+"&dateVal="+hidVal+"&calbase="+calbase,"Calendar",225,160,15,12);
