...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT030726") + '"	onClick="javascript:return caclhm_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function caclhm_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('caclhm_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('caclhm_res',obj,'ONCLICK') == false) { 
		return false;
	}
