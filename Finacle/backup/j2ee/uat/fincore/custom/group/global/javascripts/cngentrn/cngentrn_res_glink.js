function printBlock()
{
	writeCustomHeader("cngentrn_res");
	with (document){
...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT031310") + '"	onClick="javascript:return cngentrn_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cngentrn_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cngentrn_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cngentrn_res',obj,'ONCLICK') == false) { 
		return false;
	}
