function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">Custom Parameter Setup Result Page</td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="Ok"	onClick="javascript:return cparam_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cparam_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cparam_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cparam_res',obj,'ONCLICK') == false) { 
		return false;
	}
