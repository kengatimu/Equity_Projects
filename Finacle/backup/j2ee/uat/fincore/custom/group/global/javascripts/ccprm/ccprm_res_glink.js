...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT030541") + '"	onClick="javascript:return ccprm_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function ccprm_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('ccprm_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('ccprm_res',obj,'ONCLICK') == false) { 
		return false;
	}
