...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT648003") + '"	onClick="javascript:return clia_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function clia_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('clia_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('clia_res',obj,'ONCLICK') == false) { 
		return false;
	}
