function cietxn_det3_post_ONLOAD()
{
        var ObjForm = document.forms[0];

	if(funcCode == "A"){
                ObjForm.debitEntity.focus();
...
function fnOnloadDisableFields(){
        var ObjForm = document.forms[0];
        ObjForm.debitEntityDesc.disabled = true;
...
function fnValidateData(){
	...
	var crAcctNumCcy = ObjForm.crAcctNumCcy.value;
	var acctCrncy = ObjForm.targetCcy.value.toUpperCase();
	...
	*/
    ...
    var acctNumber = ObjForm.creditEntity.value;
    if(!fnIsNull(creditEntity))
    {   
        // alert("No 2");
...
function getAcctIdList1() {
        var ObjForm = document.forms[0];
        var debitEntity = ObjForm.debitEntity.value;
	cust_showAccountIdList1(ObjForm.drAcctNum,null,null,'F');
        /*if(fnIsNull(debitEntity)){
                alert("Enter the Debit A/c. Entity ID to search for Debit Account ID");
                ObjForm.debitEntity.focus();
        }else{
                cust_showAccountIdList1(ObjForm.drAcctNum,null,null,'F');
        }
...
function fnGetDrAcctDetails(){
	...
	if(!fnIsNull(drAcctNum)){
		...
		fnClearAmtFields();
		fnClearRateFields();
		ObjForm.crAcctNumEntityId.focus();
        }else{
                fnClearAcctDetails1();
        }
        return true;
...
if(fnIsNull(ObjForm.creditEntity.value))
{
	alert("Credit Entity Id should be entered first");
...
//			document.forms[0].varbiotest.value = getSValue("bioStatus");
        ...
        var targetEntity = ObjForm.debitEntity.value;

        if(fnIsNull(targetEntity)){
                ObjForm.targetEntityDesc.value = "";
        }else{
                inputNameValues = "targetEntity|"+targetEntity;
                outputNames = "errorFlg|errorMsg|targetEntity|targetEntityDesc|boxFlg";
                scriptName = "cietxndp018.scr";
                retVal = appFnExecuteScript(inputNameValues,outputNames,scriptName,false);
                token = retVal.split("|");
                if(token != undefined){
                        errorFlg = token[1];
                        errorMsg = token[3];
...
document.forms[0].varbiotest.value = getSValue("bioStatus");
	if (profileId=="50"){
		...
		if(document.forms[0].varbiotest.value == "R"){
			alert("BIO Override Not Allowed for CIETXN Rwanda");
...
function fnGetAcctDetails2(){
        ...
        var crAcctNum = ObjForm.crAcctNum.value;
        var inputNameValues = "acctNum|"+crAcctNum+"|targetEntity|"+ObjForm.creditEntity.value+"|txnType|TF";
...
function fnGetNostroVostroAccts(){
	var ObjForm = document.forms[0];
        var debitEntity = ObjForm.drAcctNumEntityId.value;
        var drAcctNum = ObjForm.drAcctNum.value;
        var creditEntity = ObjForm.crAcctNumEntityId.value;
        var crAcctNum = ObjForm.crAcctNum.value;
	...
	var inputNameValues = "debitEntity|"+debitEntity+"|creditEntity|"+creditEntity+"|drAcctNum|"+drAcctNum+"|crAcctNum|"+crAcctNum+"|acctNumSolId|"+ObjForm.crAcctNumSolId.value+"|acctNumCcy|"+ObjForm.crAcctNumCcy.value;
        var outputNames = "errorFlg|errorMsg|nostroAcct|nostroAcctFlg|vostroAcct|vostroAcctFlg|targetCcy|nostroAcctName";
        var scriptName = "cietxndp028.scr";
        var retVal = appFnExecuteScript(inputNameValues,outputNames,scriptName,false);
        var token = retVal.split("|");
        if(token != undefined){
                var errorFlg = token[1];
		...
		var targetCcy = token[13];
		var nostroAcctName = token[15];

                if(errorFlg == "Y"){
                        alert(errorMsg);
			if(errorMsg == "Same Entity Transaction Not Allowed"){
                                fnClearAcctDetails2();
                                ObjForm.crAcctNum.value = "";
                                ObjForm.creditEntity.value = "";
                                ObjForm.creditEntityDesc.value = "";
				ObjForm.creditEntity.focus();
                                return false;
                        }
				...
				ObjForm.nostroAcctName.value = "";
                        ...
                        ObjForm.nostroAcct.value = nostroAcct;
                        ObjForm.vostroAcct.value = vostroAcct;
			ObjForm.targetCcy.value = targetCcy;
			ObjForm.nostroAcctName.value = nostroAcctName;
                }
        }
        return true;
...
function fnClearAcctDetails1(){
	...
	document.forms[0].varbiotest.value == = "";
	fnClearAmtFields();
	fnClearRateFields();
ObjForm.drAcctNum.focus();
...
function fnGetChargeAmt(){
	...
	if(fnIsNull(drAcctNumEntityId)){
		...
		//var inputNameValues="creditEntity|"+crAcctNumEntityId+"|drAcctNumEntityId|"+drAcctNumEntityId+"|txnType|TF|refCrncy|"+refCrncy+"|refAmt|"+refAmt+"|rateCode|"+rateCode+"|rate|"+rate+"|drAcctNum|"+drAcctNum+"|crAcctNum|"+crAcctNum;
		var inputNameValues="crAcctNumEntityId|"+crAcctNumEntityId+"|drAcctNumEntityId|"+drAcctNumEntityId+"|txnType|TF|refCrncy|"+refCrncy+"|refAmt|"+refAmt+"|rateCode|"+rateCode+"|rate|"+rate+"|drAcctNum|"+drAcctNum+"|crAcctNum|"+crAcctNum+"|acctNumCcy|"+ObjForm.crAcctNumCcy.value;
...
function fnsubmitchek(){
	...
	var crEntityId = ObjForm.creditEntity.value;
	var crUserId = "FIVUSR"+ObjForm.creditEntity.value;
	var TranPart  =  "CREDIT TRANSFER THRU%";
...
//Show Account Id list js function For Debit Account ID

function cust_showAccountIdList1(acctObj,solId,acctName,inPreceedence,currDesc,defCrncyCode,defSchemeCode,defSchemeType,defSolId,defCifId,defGLSubHead,defOwnership,defShortName,defAcctLbl,defPartn,doAutoSubmit,defMasterAcctId,searchLang,targetBankId,dispBankIDFldFlg){
...
/* Assigning default if passed */
	if(acctObj.value != ""){acctIdVal = acctObj.value;}
        ...
        //strTargetBankId = document.forms[0].debitEntity.value;
...
//Show Account Id list js function For Credit Account ID
...
function showFinacleSignature(){
     ...
     var signUrl = bUrl + "/finbranch/GetSignature?EntityId=" + acctBnkId + "&CustId=" + cifIdForSig + "&AcctNo=" + acctNo + "&SessId=1";
     //window.open(signUrl);
	window.showModalDialog(signUrl,"","status=no;help=no;addressbar=no;location=no;menubar=no;resizable=no;dialogHeight=850px;dialogWidth=850px");
}
...
/*function cietxn_det3_post_ONCHANGE(obj)
{
        //alert("Post Change");
        if(obj.id == "drAcctNum")
        {
...
function fnValidateAcctNumEntityId(obj1,obj2){
	var ObjForm = document.forms[0];
	var inputNameValues = "txnType|TF|acctNum|"+obj1+"|fieldValidated|"+obj2;
	var outputNames = "errorFlg|errorMsg";
...
//if(obj.id == "refCrncy")
//{
var cracctCrncy = document.forms[0].crAcctNumCcy.value;

var dracctCrncy =document.forms[0].drAcctNumCcy.value;

var refCrncy = document.forms[0].refCrncy.value;
var inputNames = "cracctCrncy|" +cracctCrncy +"|dracctCrncy|" + dracctCrncy +"|refCrncy|" + refCrncy;
var outputNames = "rateCode|rate|treaRate|errorFlg|errorMsg";
var scr="cietxnRefdp002.scr";
...
function cietxn_det3_pre_ONCLICK(obj)
{
	...
	if(obj.id == "Submit")
	{
		if(treaRefNum != "")
		{
			...
			var refAmt = removeCommas(refAmtTmp);
                	var inputNames = "refCrncy|" + refCrncy + "|rateCode|"+rateCode+"|rate|"+rate+"|treaRate|"+treaRate+"|treaRefNum|"+treaRefNum+"|tranType|T"+"|crAcctNum|"+crAcctNum+"|drAcctNum|"+drAcctNum+"|refAmt|"+refAmt;
                	var outputNames = "errorFlg|errorMsg";
