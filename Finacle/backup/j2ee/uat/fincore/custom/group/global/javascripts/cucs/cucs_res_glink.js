...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT648010") + '"	onClick="javascript:return cucs_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cucs_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cucs_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cucs_res',obj,'ONCLICK') == false) { 
		return false;
	}
