function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">Customer Relationship Manager Maintenance</td>');
	write('</tr>');
	write('</table>');
	...
	write('<tr>');
	write('<a href="javascript:showHelpFile(\'crmupl_det_help.htm\');" id="sLnk1">');
	...
	write('<tr>');
		...
		write('<td><select name="' + subGroupName + '.funCode" id="funCode" ' + crmuplProps.get("funCode_ENABLED") + ' class="textfieldfont" " onchange="javascript:return funcCodeOnchange()">'); 
	     //write('<td><select name="' + subGroupName + '.funCode" id="funCode" ' + crmuplProps.get("funCode_ENABLED") + ' class="textfieldfont" " onchange="javascript:fnDisableFields(this)">');
		...
		write('<option value="M">' + jspResArr.get("FLT026346") + '</option>');
	...
	write("<tr id='uploadrow'>");
		...
		write('<td><input name="' + subGroupName + '.uploadFileName" id="uploadFileName" ' + crmuplProps.get("uploadFileName_ENABLED") + ' type="file" class="textfieldfont" size="50" maxlength="100" >');
	...
	write("<tr id='refcode'>");
		...
		write('<td><input name="' + subGroupName + '.refnumber" id="refnumber" ' + crmuplProps.get("refnumber_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" >');
...
function printFooterBlock()
{
	...
	write('<input name="Submit" type="button" class="button" onClick="javascript:return crmupl_det_ONCLICK1(this,this);" id="Submit" value="Submit" hotKeyId="Submit">');
	}else{
...
function crmupl_det_ONCLICK1(obj,p1)
{
	var retVal = "";
	if (pre_ONCLICK('crmupl_det',obj) == false) { 
		return false;
	}
	...
	if (post_ONCLICK('crmupl_det',obj) == false) { 
	return false;
	}
...
function crmupl_det_ONCLICK2(obj)
{
	var retVal = "";
	if (pre_ONCLICK('crmupl_det',obj) == false) { 
		return false;
