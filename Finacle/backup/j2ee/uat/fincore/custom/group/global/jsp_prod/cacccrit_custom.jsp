...
function fnShowClsAccountIdList(funcCode)
{
	if(objForm.funcCode.value == VERIFY_MODE || objForm.funcCode.value == CANCEL_MODE)
        {
                var funcCode = objForm.funcCode.value;
                showDynSearcher('ACCLCHRG','',':acctId=acctId');
        }
        else
        {
...
function cacccrit_ONCHANGE5(obj)
{
        var retVal = "";
        if (preEventCall('cacccrit',obj,'ONCHANGE') == false) {
                return false;
        }
        if ((retVal = fnFetchDrAcctId()) == false) {
                return false;
        }
        if (postEventCall('cacccrit',obj,'ONCHANGE') == false) {
                return false;
        }
		memoPad();
