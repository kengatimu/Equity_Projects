...
if(mopId == "HBGPRINT") 
	...
	document.forms[0].mrt_name.value="bgprint.mrt";
        ...
        if (mopId == "HPPT") {
              //alert("Disable  normal posting");
                var radioButtons = document.getElementsByName("proxy_post_flg");
                radioButtons[0].checked = false;
                radioButtons[1].checked = true;
...
if (mopId == "HTTUM")
        {
        var radioButtons = document.getElementsByName("fire_bfti_flg");
...
if (profileId=="11")
{
// Check Document Expiration Dates -- Anderson ---------
function batchcriteria_pre_ONCLICK(obj){
    if(mopId == "HPSP"){
        if ((obj.id == "Validate") || (obj.id == "Submit")){
            var acctNumAlrt = document.forms[0].low_acct_num.value;
            if(acctNumAlrt != ""){
                var inputNameValues = "foracid|" + acctNumAlrt;
                var outputNames = "msgFlg";
                var scrName = "checkDocExpDate.scr";
                var retVal = appFnExecuteScript(inputNameValues, outputNames, scrName, false);
                var resp = retVal.split("|");
                if (resp[1]) {
                    alert(resp[1]);
                }
            }
...
// End Check Document Expiration Dates -- Anderson ---------
}
...
function batchcriteria_pre_ONCLICK(obj)
{
	...
	if((obj.id == 'Submit') || (obj.id == 'Validate'))
	{
		...
		if (profileId=="50")
		{
			if(mopId == "HLADGEN")
			{
				if(document.forms[0].acct_from.value =="")
				 {
					 alert ("Please Enter A/c from");
					 return false;
				 }
				if (document.forms[0].acct_from.value!=document.forms[0].acct_to.value)
					{
