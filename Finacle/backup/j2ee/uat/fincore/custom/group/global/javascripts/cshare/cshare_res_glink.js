function printBlock()
{
	writeCustomHeader("cshare_res");
	with (document){
	write('<input type="hidden" name="cshare.TradingResult" id="TradingResult">');
	write('<input type="hidden" name="cshare.total" id="total">');
	write('<input type="hidden" name="cshare.share" id="share">');
	write('<input type="hidden" name="cshare.qty" id="qty">');
...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT030726") + '"	onClick="javascript:return cshare_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cshare_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cshare_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cshare_res',obj,'ONCLICK') == false) { 
		return false;
	}
