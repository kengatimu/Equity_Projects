function printBlock()
{
	writeCustomHeader("hdentill_res");
	with (document){
	...
	write('</img></td>');
	if(Message != "NULL"){
	write('<td width="97%" style="color:red"> ' + Message + '</td>');
	}
	else if(Message == ""){
...
function printFooterBlock()
{
	with (document) {
	write('<span class="cform">');
	write('<div class="ctable" align="center">');
	write('<input type="button" class="button" id="ok" value="Accept" onClick="javascript:return hdentill_res_ONCLICK1(this);" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	write('</span>');
	}
...
function hdentill_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('hdentill_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('hdentill_res',obj,'ONCLICK') == false) { 
		return false;
	}
