function printBlock()
{
	...
	write('<select name="' + subGroupName + '.funcCode" id="funcCode" ' + cietxnProps.get("funcCode_ENABLED") + ' class="listboxfont" onChange="javascript: fnOnChangeFuncCode();">');
	...
	write('<td class="textlabel">' + jspResArr.get("FLT021243") + '<script>setMandatory("Y");</script></td>');
        write('<td>');
        write('<select name="' + subGroupName + '.txnType" id="txnType" ' + cietxnProps.get("txnType_ENABLED") + ' class="listboxfont" style="width:260px">');
        write('<option value="">' + jspResArr.get("FLT021247") + '</option>');
        write('<option value="CD">' + jspResArr.get("FLT021258") + '</option>');
        write('<option value="CW">' + jspResArr.get("FLT021259") + '</option>');
        //write('<option value="TT">' + jspResArr.get("FLT021260") + '</option>');
	write('<option value="TF">' + jspResArr.get("FLT021265") + '</option>');
	//write('<option value="RF">' + jspResArr.get("FLT021266") + '</option>');
	write('<option value="PA">' + jspResArr.get("FLT021261") + '</option>');
        write('</select>');
        write('</td>');
	write('</tr>');
	...
	write('<tr id="ieTxnRefNoRow">');
        ...
        write('<td class="textfield">');
        write('<input maxlength="10" type="text" class="textfieldfont" name="' + subGroupName + '.ieTxnRefNo" id="ieTxnRefNo" ' + cietxnProps.get("ieTxnRefNo_ENABLED") + ' onChange="javascript: cietxn_crit_ONCHANGE1()">');
	...
	/*write('<tr id="tranIdRow">');
        ...
        write('<td class="textfield">');
        write('<input maxlength="10" type="text" class="textfieldfont" name="' + subGroupName + '.tranId" id="tranId" ' + cietxnProps.get("tranId_ENABLED") + '>');
	...
	*/
	...
	write('<tr id="tranDateRow">');
        ...
        write('<td class="textfield">');
        write('<input  maxlength="10" hotKeyId="calender1"  type="text" class="textfieldfont" fdt="uidate"  mnebl="false" onBlur="javascript:return cietxn_crit_ONBLUR1(this,this,this);" name="' + subGroupName + '.tranDate_ui" id="tranDate_ui" >');
...
function printFooterBlock()
{
	...
	write('<div align="left" class="ctable">');
	write('<input id="Accept" name="Go" type="button" class="button" onClick="javascript:return cietxn_crit_ONCLICK1(this,this);"" value="Go" hotKeyId="Go">');
	write('<input id="Clear" name="Clear" type="button" class="button" value="Clear" onClick="javascript:return cietxn_crit_ONCLICK2(this);"">');
	writeFooter();
	write('</div>');
	}
	} //End with()
...
function cietxn_crit_ONCLICK1(obj,p1)
{
	var retVal = "";
	if (preEventCall('cietxn_crit',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal =  fnValAndSubmit(p1)) == false) {
		return false;
	}
	if (postEventCall('cietxn_crit',obj,'ONCLICK') == false) { 
		return false;
	}
	return (retVal == undefined) ? true : retVal;
}

function cietxn_crit_ONCLICK2(obj)
{
	var retVal = "";
	if (preEventCall('cietxn_crit',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal =  fnClearAllFields()) == false) {
		return false;
	}
	if (postEventCall('cietxn_crit',obj,'ONCLICK') == false) { 
		return false;
...
function cietxn_crit_ONBLUR1(obj,p1,p2)
{
        var retVal = "";
        if (preEventCall('cietxn_crit',obj,'ONBLUR') == false) {
                return false;
        }
        ...
        if (postEventCall('cietxn_crit',obj,'ONBLUR') == false) {
                return false;
        }
        return (retVal == undefined) ? true : retVal;
}
