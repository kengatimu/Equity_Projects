function printBlock()
{
	...
	write('</tr>');
	write('<tr>');
	write('<td class="page-heading">Risk Rating Maintenance</td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<input type="button" class="button" id="ok" value="Ok" onClick="javascript:return ccrrm_res_ONCLICK1(this);" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function ccrrm_res_ONCLICK1(obj)
{
	var retVal = "";
	/*
	if (pre_ONCLICK('ccrrm_res',obj) == false) { 
		return false;
	}
	*/
	...
	/*
	if (post_ONCLICK('ccrrm_res',obj) == false) { 
		return false;
	}
	*/
