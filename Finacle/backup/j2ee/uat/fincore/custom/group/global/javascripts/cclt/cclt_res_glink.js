function printBlock()
{
	...
	write('</tr>');
	write('<tr>');
	write('<td class="page-heading">CLT Maintenance</td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<input type="button" class="button" id="ok" value="Ok" onClick="javascript:return cclt_res_ONCLICK1(this);" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cclt_res_ONCLICK1(obj)
{
	var retVal = "";
	/*
	if (pre_ONCLICK('cclt_res',obj) == false) { 
		return false;
	}
	*/
	...
	/*
	if (post_ONCLICK('clt_res',obj) == false) { 
		return false;
	}
	*/
