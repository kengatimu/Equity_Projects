function printBlock()
{
	writeCustomHeader("catodapp_res");
	with (document){
	...
	write('<tr>');
	write('<td class="page-heading">CREDT SCORE MAINT </td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<div class="ctable">');
        write('<input type="button" class="button" id="ok" value="Ok" onClick="javascript:return catodapp_res_ONCLICK1(this);" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function catodapp_res_ONCLICK1(obj)
{
	var retVal = "";
	/*
	if (pre_ONCLICK('catodapp_res',obj) == false) { 
		return false;
	}
	*/
	...
	/*
	if (post_ONCLICK('catodapp_res',obj) == false) { 
		return false;
	}
	*/
