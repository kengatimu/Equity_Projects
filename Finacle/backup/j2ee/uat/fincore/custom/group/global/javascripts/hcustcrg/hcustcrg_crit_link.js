...
function fnShowEvtIDList(a)
{
		...
		if (ObjForm.evtType.value !="")
		{
			...
			var eventType = document.forms[0].evtType.value.toUpperCase();
			getEvtIdList(document.forms[0].evtId,'ctrl','F','NULL','N','Y',eventType);
...
function staffIdSearcher()
{
		...
		var pageTitle         = "Employer Id List";
		var scrName           = "HCUSTCRD_EMP_Searcher.scr";
		var hyperLnkCols      = "1";
		var literalNames      = "Employer Id|Empployer Name";
...
function fncommensearcher()
{
	...
	var Level    = objForm.Level.value;
		...
		if (Level == "A")
		{
			var outputNames       = "acctNum|evtType|evtId";
			var pageTitle         = "Account No List ";
			var literalNames      = "Account No | Event Type | Event Id";
		}
		if (Level == "C")
                {
                        var outputNames       = "cifId|evtType|evtId";
                        var pageTitle         = "Cif Id List ";
                        var literalNames      = "Cif Id | Event Type | Event Id";
                }

		if (Level == "AG")
                {
                        var outputNames       = "EmpId|evtType|evtId";
                        var pageTitle         = "Agreement List ";
                        var literalNames      = "Agreement | Event Type | Event Id";
                }
...
function evtidevtypsearch()
{
        var objForm = document.forms[0];
        var funcCode = objForm.funcCode.value;
        var Level    = objForm.Level.value;
        var evtid    = objForm.evtId.value;
        var evttyp   = objForm.evtType.value;
        if (objForm.funcCode.value != "")
        {
                if (Level == "A")
                {
                        var acctno            = objForm.acctNum.value;
                        var outputNames       = "acctNum|evtType|evtId";
                        var literalNames      = "Account No | Event Type | Event Id";
                        var inputNameValues   = "funcCode|"+funcCode+"|Level|"+Level+"|acctno|"+acctno;
                }
                if (Level == "C")
                {
                        ...
                        var outputNames       = "cifId|evtType|evtId";
                        var literalNames      = "Cif Id | Event Type | Event Id";
                        var inputNameValues   = "funcCode|"+funcCode+"|Level|"+Level+"|cifId|"+cifId;
                }

                if (Level == "AG")
                {
                        var empid             =objForm.EmpId.value; 
                        var outputNames       = "EmpId|evtType|evtId";
                        var literalNames      = "Agreement | Event Type | Event Id";
                        var inputNameValues   = "funcCode|"+funcCode+"|Level|"+Level+"|empid|"+empid;
                }
                        var pageTitle         = "Event Type / Event Id Commen Searcher "; 
                        var scrName           = "HCUSTCRD_custom_evtidtyp_Searcher.scr";
                        var hyperLnkCols      = "1";
                        var isPopulationReq   = "true";
                        var retVal            = fnExecuteScriptForList(inputNameValues,outputNames,scrName,pageTitle,literalNames,hyperLnkCols,isPopulationReq);
        }
...
function hcustcrg_det_ONCHANGE5 ()
{
        if(document.forms[0].evtType.value!="")
        {
                var input = "evtType|" + document.forms[0].evtType.value;
                var output = "count";
                var scrName = "HCUSTCRD_Evnttype.scr";
                var outputNames = "";
                var retVal = appFnExecuteScript(input,output,scrName,false);
                var retBuff = retVal.split("|");
                if (retBuff[0] == "S")
                {
                        document.forms[0].evtTypeDesc.value = retBuff[1];
                        document.forms[0].evtTypeDesc.disabled = true;
                }
                if (retBuff[0] == "F")
                {

                        alert("Invalid Event Type.");
                         document.forms[0].evtType.value = "";
                        document.forms[0].evtType.focus();
                        return false
                }
        }
