function cierev_det_post_ONLOAD()
{
...
function fnGetEntityDesc1(){
        ...
        if(fnIsNull(debitEntity)){
                ...
                var scriptName = "cierevdp002.scr"
...
function fnGetAcctDetails1(){
	...
	var scriptName = "cierevdp001.scr"
        var retVal = appFnExecuteScript(inputNameValues,outputNames,scriptName,false);
        var token = retVal.split("|");
	if(token != undefined){
		var errorFlg = token[1];
...
function fnExplodeBalanceDetails(obj){
	...
	if(obj.id == "drAcctNum"){
        ...
        var scriptName = "cierevdp003.scr"
        var retVal = appFnExecuteScript(inputNameValues,outputNames,scriptName,false);
        var token = retVal.split("|");
        if(token != undefined){
                var iIndex = 1;
        ...
        var sUrl = "../custom/jsp/cierev_acctBal.jsp?";
        sUrl+= urlData;
        //alert("sUrl = "+sUrl);
