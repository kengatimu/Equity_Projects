...
function printFooterBlock(){
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT648011") + '"	onClick="javascript:return ebrpt_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function

function ebrpt_res_ONCLICK1(obj){
	var retVal = "";
	if (pre_ONCLICK('ebrpt_res',obj) == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (post_ONCLICK('ebrpt_res',obj) == false) { 
		return false;
	}
