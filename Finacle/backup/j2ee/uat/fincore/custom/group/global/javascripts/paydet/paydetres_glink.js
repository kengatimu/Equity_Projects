function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">' + jspResArr.get("FLT031834") + '</td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT031295") + '"	onClick="javascript:return paydetres_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function paydetres_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('paydetres',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('paydetres',obj,'ONCLICK') == false) { 
		return false;
	}
