...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT030541") + '"	onClick="javascript:return cprint_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cprint_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cprint_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cprint_res',obj,'ONCLICK') == false) { 
		return false;
	}
