function printBlock()
{
	writeCustomHeader("hcustcrg_res");
	with (document){
 	//write('<input type="hidden" id="pymtType" mneb1="N"  name="' + subGroupName + '.pymtType">');
...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT004977") + '"	onClick="javascript:return hcustcrg_res_ONCLICK1(this);"" hotKeyId="Go">');
	//write('<input id="print" name="print" type="button" class="button" value="' + jspResArr.get("FLT001439") + '"	onClick="javascript:return hcustcrg_res_ONCLICK2(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function hcustcrg_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('hcustcrg_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('hcustcrg_res',obj,'ONCLICK') == false) { 
		return false;
	}
