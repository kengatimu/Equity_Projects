function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">STAFF SALARIES PRE-PROCESSOR</td>');
	write('</tr>');
	write('</table>');
	...
	write('<tr>');
	write('<a href="javascript:showHelpFile(\'cssalpp_det_help.htm\');" id="sLnk1">');
	...
	write('<td><input name="' + subGroupName + '.funcCode" id="funcCode" ' + cssalppProps.get("funcCode_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" disabled="true">');
...
//    write('</select>');
	...
	write('<td><input name="' + subGroupName + '.setId" id="setId" ' + cssalppProps.get("setId_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" disabled="true">');
//	write('<a target=_self id="sLnk1" href="javascript:showSetList(document.forms[0].setId,\'ctrl\',\'F\',\' \' )"><img class="img" src="../Renderer/images/'+ applangcode + '/search_icon.gif" width="16" height="17" border="0" hotKeyId="search9"></img></a>');
	...
	write('<td><input type="text" class="textfieldfont" id="salDate" name="' + subGroupName + '.salDate" fdt="uidate" mnebl="false" fmnd="Y"  fblk="defaultFblk1" hotKeyId="calender1" readonly="true">');
	write('</td>');
    write('<td>&nbsp;</td>');
    write('<td>&nbsp;</td>');
    write('<td>&nbsp;</td>');
	...
	write('<td><input name="' + subGroupName + '.remrks" id="remrks" ' + cssalppProps.get("remrks_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16">');
	write('</td>');
    write('<td>&nbsp;</td>');
    write('<td>&nbsp;</td>');
    write('<td>&nbsp;</td>');
...
function printFooterBlock()
{
	...
	write('<input name="Submit" type="button" class="button" onClick="javascript:return cssalpp_det_ONCLICK1(this,this);" id="Submit" value="Submit" hotKeyId="Submit">');
	write('<input name="Clear" type="button" class="button" onClick="javascript:return cssalpp_det_ONCLICK2(this);" id="Clear" value="Clear">');
	}else{
...
{
	var ObjForm = document.forms[0];
	ObjForm.funcCode.value = "P-Print";
...
function cssalpp_det_ONCLICK1(obj,p1)
{
	var retVal = "";
	if (pre_ONCLICK('cssalpp_det',obj) == false) { 
		return false;
	}
	...
	if (post_ONCLICK('cssalpp_det',obj) == false) { 
	return false;
	}
	return (retVal == undefined) ? true : retVal;
}

function cssalpp_det_ONCLICK2(obj)
{
	var retVal = "";
	if (pre_ONCLICK('cssalpp_det',obj) == false) { 
		return false;
