...
//    write('<td class="textfielddisplaylabel">'+instNum+'</td>');
	...
	write('<td><input name="' + subGroupName + '.beginNum" id="beginNum" ' + cuicsauProps.get("beginNum_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" >');
	write('</td>');
	write('<td class="textlabel">' + jspResArr.get("FLT026335") + '<font color=red size=2>*</font></td>');
	write('<td><input name="' + subGroupName + '.newStatus" id="newStatus" ' + cuicsauProps.get("newStatus_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" >');
	write('</td>');
	write('</tr>');
	write('<tr>');
	/*write('<td class="textlabel">' + jspResArr.get("FLT026337") + '<font color=red size=2>*</font></td>');
	write('<td><input name="' + subGroupName + '.numLeaves" id="numLeaves" ' + cuicsauProps.get("numLeaves_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" >');
	write('</td>');*/
	
	write('<td class="textlabel">' + jspResArr.get("FLT026334") + '<font color=red size=2>*</font></td>');
	write('<td><input name="' + subGroupName + '.currStatus" id="currStatus" ' + cuicsauProps.get("currStatus_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" >');
	write('</td>');
	write('</tr>');
	write('<tr>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<input name="Submit" type="button" class="button" onClick="javascript:return cuicsau_det_ONCLICK1(this,this);" id="Submit" value="Submit" hotKeyId="Submit">');
	//write('<input name="Clear" type="button" class="button" onClick="javascript:return cuicsau_det_ONCLICK2(this);" id="Clear" value="Clear">');
	write('<input name="Cancel" type="button" class="button" onClick="javascript:return cuicsau_det_ONCLICK2(this);" id="Clear" value="Cancel">');
	}else{
...
function cuicsau_det_ONCLICK1(obj,p1)
{
	var retVal = "";
	...
	*/
	if (pre_ONCLICK('cuicsau_det',obj) == false) { 
		return false;
	}
	...
	if (post_ONCLICK('cuicsau_det',obj) == false) { 
	return false;
	}
...
function cuicsau_det_ONCLICK2(obj)
{
	var retVal = "";
	if (pre_ONCLICK('cuicsau_det',obj) == false) { 
		return false;
...
function cuicsau_det_ONCLICK3(obj)
{
	var retVal = "";
	if (pre_ONCLICK('cuicsau_det',obj) == false) { 
		return false;
