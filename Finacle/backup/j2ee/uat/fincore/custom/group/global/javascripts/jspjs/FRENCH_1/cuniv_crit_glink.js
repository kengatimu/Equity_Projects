function printBlock()
{
	...
	write('<td><select name="' + subGroupName + '.funcCode" id="funcCode" ' + cunivProps.get("funcCode_ENABLED") + ' class="textfieldfont" " onchange="javascript:fnDisableFields(this)">');
...
function printFooterBlock()
{
    ...
    if (sReferralMode == 'S'){
    write('<input type="button" class="Button" id="Envoyer  (soumettre) " value="SUBMIT" onClick="javascript:return doRefSubmit(this);" hotKeyId="Envoyer  (soumettre) " >');
    }
    writeRefFooter();
    write('<input type="button" class="Button" id="_BackRef_" value="ANNULER" onClick="javascript:return doSubmit(this.id);" hotKeyId="Annuler /Effacer " >');
    write('</div>');
    }else{
     if(funcCode !='I'){   
	write('<div class="ctable">');
    write('<input name="FLT023614: Accepter" type="button" class="button" onClick="javascript:return cuniv_crit_ONCLICK1(this,this);" id="FLT023614: Accepter" value=" (Aller)" hmtKeyId="Envoyer  (soumettre) ">');
    }else{
...
function cuniv_crit_ONCLICK1(obj,p1)
{
    var retVal = "";
