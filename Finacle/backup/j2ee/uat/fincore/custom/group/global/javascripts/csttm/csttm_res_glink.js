function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">SHARES TRADING TRANSACTION MAINTENANCE SCREEN</td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<input type="button" class="button" id="ok" value="Ok" onClick="javascript:return csttm_res_ONCLICK1(this);" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function csttm_res_ONCLICK1(obj)
{
	var retVal = "";
	/*
	if (pre_ONCLICK('cccpfgm_res',obj) == false) { 
		return false;
	}
	*/
	...
	/*
	if (post_ONCLICK('cccpfgm_res',obj) == false) { 
		return false;
	}
	*/
