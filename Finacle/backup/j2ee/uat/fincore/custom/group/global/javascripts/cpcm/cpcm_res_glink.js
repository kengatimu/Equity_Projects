...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT648005") + '"	onClick="javascript:return cpcm_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
...
function cpcm_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cpcm_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cpcm_res',obj,'ONCLICK') == false) { 
		return false;
	}
