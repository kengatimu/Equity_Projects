function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">Customer CLT ID Maintenance.</td>');
	write('</tr>');
	write('</table>');
	...
	write('<tr>');
	write('<a href="javascript:showHelpFile(\'cclt_det_help.htm\');" id="sLnk1">');
	...
	write('<tr>');
		...
		write('<td><select name="' + subGroupName + '.funCode" id="funCode" ' + ccltProps.get("funCode_ENABLED") + ' class="textfieldfont" " onchange="javascript:return funcCodeOnchange()">'); 
	    //write('<td><select name="' + subGroupName + '.funCode" id="funCode" ' + cltProps.get("funCode_ENABLED") + ' class="textfieldfont" " onchange="javascript:fnDisableFields(this)">');
	...
	write("<tr id='uploadrow'>");
		...
		write('<td><input name="' + subGroupName + '.uploadFileName" id="uploadFileName" ' + ccltProps.get("uploadFileName_ENABLED") + ' type="file" class="textfieldfont" size="50" maxlength="100" >');
	...
	write("<tr id='refcode'>");
		...
		write('<td><input name="' + subGroupName + '.refnumber" id="refnumber" ' + ccltProps.get("refnumber_ENABLED") + ' type="text" class="textfieldfont" size="25" maxlength="16" >');
...
function printFooterBlock()
{
	...
	write('<input name="Submit" type="button" class="button" onClick="javascript:return cclt_det_ONCLICK1(this,this);" id="Submit" value="Submit" hotKeyId="Submit">');
	//write('<input name="Clear" type="button" class="button" onClick="javascript:return clt_det_ONCLICK2(this);" id="Clear" value="Clear">');
	}else{
...
function cclt_det_ONCLICK1(obj,p1)
{
	var retVal = "";
	if (pre_ONCLICK('cclt_det',obj) == false) { 
		return false;
	}
	...
	if (post_ONCLICK('cclt_det',obj) == false) { 
	return false;
	}
...
function cclt_det_ONCLICK2(obj)
{
	var retVal = "";
	if (pre_ONCLICK('cclt_det',obj) == false) { 
		return false;
