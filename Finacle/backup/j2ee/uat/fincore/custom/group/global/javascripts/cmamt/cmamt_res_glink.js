function printBlock()
{
	...
	write('<tr>');
	write('<td class="page-heading">' + jspResArr.get("FLT031764") + '</td>');
	write('</tr>');
	write('</table>');
	write('</td>');
	write('</tr>');
...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT030726") + '"	onClick="javascript:return cmamt_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cmamt_res_ONCLICK1(obj)
{
	var retVal = "";
	if (preEventCall('cmamt_res',obj,'ONCLICK') == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (postEventCall('cmamt_res',obj,'ONCLICK') == false) { 
		return false;
	}
