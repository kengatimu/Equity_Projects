...
ADT.audit_bod_date||'|'||substr(table_key,0,instr(table_key,'/')-1) FROM TBAADM.GAM,TBAADM.ADT,TBAADM.ATT
WHERE GAM.acid = ATT.acid 
AND substr(table_key,0,instr(table_key,'/')-1) = ATT.SRL_NUM
AND ADT.init_sol_id in (SELECT sol_id FROM TBAADM.SST WHERE set_id = '&1' AND bank_id = '&2')
AND ADT.table_name = 'ATT'
AND ENTERER_ID not in ('SK04908','JN05794')
AND ADT.auth_id = '!'
AND GAM.bank_id = ADT.bank_id
AND GAM.bank_id = ATT.bank_id
AND GAM.bank_id = '&2';
spool off;
exit;
