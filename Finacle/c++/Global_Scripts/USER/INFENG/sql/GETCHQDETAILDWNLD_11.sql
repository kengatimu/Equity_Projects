...
CREATE OR REPLACE PACKAGE custom.Getchqdetaildwnld_11 AS

PROCEDURE Getchqdetaildwnld(inp_str IN VARCHAR2,
				out_retCode OUT NUMBER,
				out_desc OUT VARCHAR2);
END Getchqdetaildwnld_11;
/
...
CREATE OR REPLACE PACKAGE BODY custom.Getchqdetaildwnld_11 AS
...
--	out_desc := '68'||'|'||BRCODE||'|'||FORACID1||'|'||CUSTNAME||'|'||'SB'||CHQLVS||'|'||'SSP'||'|'||vCHQBOOKS||'|'||vCHSSR||'|'||vENDNO1||'|'||vREQDATE ;
...
END Getchqdetaildwnld_11;
/
GRANT EXECUTE, DEBUG ON CUSTOM.GETCHQDETAILDWNLD_11 TO TBAADM;
GRANT EXECUTE, DEBUG ON CUSTOM.GETCHQDETAILDWNLD_11 TO TBAGEN;
GRANT EXECUTE, DEBUG ON CUSTOM.GETCHQDETAILDWNLD_11 TO TBAUTIL;
