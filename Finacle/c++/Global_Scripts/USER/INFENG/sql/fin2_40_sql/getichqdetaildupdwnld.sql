CREATE OR REPLACE PACKAGE GETICHQDETAILDUPDWNLD AS
PROCEDURE GETICHQDETAILDUPDWNLD( inp_str IN VARCHAR2,
            out_retCode OUT NUMBER,
            out_desc OUT VARCHAR2);
END GETICHQDETAILDUPDWNLD;
/

CREATE OR REPLACE PACKAGE BODY GETICHQDETAILDUPDWNLD AS
		...
		CHQALPHA		CUSTOM.C_ICBM.BEGIN_CHQ_ALPHA%type;
		...
		select  distinct C_ICBM.req_id,C_ICBM.foracid,bank_id
		from    CUSTOM.C_ICBM
...
begin
	select	RCRE_USER_ID,AUTH_USER_ID,lpad(NUM_OF_CHQ_LVS,3,0),NUM_OF_CHB,BEGIN_CHQ_NUM,REQ_DATE,DOWNLOAD_ID,BEGIN_CHQ_ALPHA
	into	RCREID,AUTHID,CHQLVS,CHQBOOKS,vCHSSR,REQDATE,DNLD_ID,vCHQALPHA
	from	CUSTOM.C_ICBM
	where	req_id = REQID
	and bank_id =bankID;
    exception
...
--	out_desc := FORACID1||'|'||CUSTNAME||'|'||BRCODE||'|'||BRNAME||'|'||BRADDRESS||'|'||vCHQBOOKS||'|'||vCHSSR||'|'||CHQLVS ;
	out_desc := vCHQBOOKS||' '||BKSTYLE||' '||vCHQALPHA||' '||vCHSSR||'    '||BANKCODE||BRCODE||CHQDIGIT||' '||FORACID1||' '||CUSTNAME||BANKCODE||BRCODE||' '||BRNAME||BRADDRESS||'  '||vREQDATE||' '||vDNLD_ID||'  ' ;

End GETICHQDETAILDUPDWNLD;

End GETICHQDETAILDUPDWNLD;
/
grant all on GETICHQDETAILDUPDWNLD to tbaadm,tbagen,tbautil
/
SHOW ERROR
/
