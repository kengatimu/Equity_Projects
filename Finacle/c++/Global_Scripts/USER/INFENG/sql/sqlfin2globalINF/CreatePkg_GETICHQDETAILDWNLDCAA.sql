...
DROP PACKAGE BODY CUSTOM.GETICHQDETAILDWNLDCAA;

CREATE OR REPLACE PACKAGE BODY CUSTOM.Getichqdetaildwnldcaa AS
CURSOR GET_ACCOUNT (STARTDATE VARCHAR2,ENDDATE VARCHAR2,SOL VARCHAR2,BID VARCHAR2) IS
SELECT  DISTINCT C_ICBM.REQ_ID,C_ICBM.FORACID
...
PROCEDURE Getichqdetaildwnldcaa1( inp_str IN VARCHAR2,
...
IF NOT Get_Account%ISOPEN THEN
    ...
    FETCH Get_Account INTO  REQID,FORACID1;

    DBMS_OUTPUT.PUT_LINE('REQID is ' ||REQID);
    IF Get_Account%NOTFOUND THEN
    DBMS_OUTPUT.PUT_LINE('REQID is inside' ||REQID);
        CLOSE Get_Account;
        out_retCode :=1;
        RETURN;
    END IF;
...
END Getichqdetaildwnldcaa1;

END Getichqdetaildwnldcaa;
/
