...
spool &4
SELECT TRIM(STAFFSALERR.solid)||'|'||TRIM(oldacc)||'|'||TRIM(amt)||'|'||TRIM(trantype)||'|'||'&5' ||'|'||errtext  FROM CUSTOM.STAFFSALERR WHERE bank_id = '&1'
/
spool off
...
SELECT TRIM(STAFFSAL.solid)||'|'||TRIM(newacc)||'|'||TRIM(amt)||'|'||TRIM(trantype)||'|'||'&5' ||'|'|| '&2' 
FROM CUSTOM.STAFFSAL , TBAADM.GAM 
wHERE GAM.foracid = CUSTOM.STAFFSAL.NEWACC
AND  GAM.bank_id = CUSTOM.STAFFSAL.bank_id
AND  GAM.bank_id = '&1'
ORDER BY srno
/
spool off
