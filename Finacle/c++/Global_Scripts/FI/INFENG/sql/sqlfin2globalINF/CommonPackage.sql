...
-----------------------------------------------------------------------------------------------------
--FUNCTION depschminterest
-----------------------------------------------------------------------------------------------------
FUNCTION depschminterest(inp_inttblcode varchar2, inp_inttblversion varchar2,inp_slabdrcrflg varchar2, inp_bankId varchar2, loc_slabamtabs varchar2, inp_period_mths varchar2, inp_period_days varchar2, inp_crncycode varchar2)
return varchar2;
...
begin
    begin
        ...
        AND bank_id = inp_bankId
        AND begin_slab_amount <= TO_NUMBER (loc_slabamtabs)
        AND TO_NUMBER (loc_slabamtabs) <= max_slab_amount
        AND (   (max_contracted_mths != 0 AND (31 * max_contracted_mths  + DECODE (max_contracted_days, 999, 30, max_contracted_days)) >= (31 * nvl(inp_period_mths,0) + nvl(inp_period_days,0)))
            OR (max_contracted_mths = 0 AND max_contracted_days >= ( 31 * nvl(inp_period_mths,0) + nvl(inp_period_days,0) ))
            )
        AND del_flg != 'Y'
        AND crncy_code = inp_crncycode
        AND entity_cre_flg = 'Y'
