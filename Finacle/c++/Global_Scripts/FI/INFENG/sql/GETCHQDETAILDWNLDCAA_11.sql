--DROP PACKAGE CUSTOM.GETCHQDETAILDWNLDCAA_1;

CREATE OR REPLACE PACKAGE CUSTOM.Getchqdetaildwnldcaa_11 AS
PROCEDURE Getchqdetaildwnldcaa( inp_str IN VARCHAR2,
            out_retCode OUT NUMBER,
            out_desc OUT VARCHAR2);
END Getchqdetaildwnldcaa_11;
/


GRANT EXECUTE, DEBUG ON CUSTOM.GETCHQDETAILDWNLDCAA_11 TO TBAADM;

GRANT EXECUTE, DEBUG ON CUSTOM.GETCHQDETAILDWNLDCAA_11 TO TBAGEN;

GRANT EXECUTE, DEBUG ON CUSTOM.GETCHQDETAILDWNLDCAA_11 TO TBAUTIL;


DROP PACKAGE BODY CUSTOM.GETCHQDETAILDWNLDCAA_11;

CREATE OR REPLACE PACKAGE BODY CUSTOM.Getchqdetaildwnldcaa_11 AS
...
--    out_desc := '681'||'|'||'68'||'|'||'2'||'|'||SUBSTR(solid,3)||'|'||SUBSTR(FORACID1,4)||'|'||CHQDIGIT||'|'||vTrancode||'|'||trim(BKSTYLE)||'|'||vCHQBOOKS||'|'||RPAD(vCHSSR,6,' ')||'|'||RPAD(CUSTNAME,80,' ')||'|'||RPAD(BRNAME,30,' ')||'|'||RPAD(BRADDRESS,30,' ')||'|'||RPAD(FORACID1,15,' ');
    out_desc := '681'||' '||'68'||'2'||SUBSTR(solid,3)||SUBSTR(FORACID1,4)||CHQDIGIT||vTrancode||trim(BKSTYLE)||vCHQBOOKS||RPAD(vCHSSR,6,' ')||RPAD(CUSTNAME,80,' ')||RPAD(BRNAME,30,' ')||RPAD(BRADDRESS,30,' ')||RPAD(FORACID1,15,' ');
