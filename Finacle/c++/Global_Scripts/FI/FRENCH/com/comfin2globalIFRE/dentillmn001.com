...
BANK_ID = $6
report=DENTILL
Report="Till Position Inquiry"
RptFile=$headersolid"_"$report"$$"
RptFile1=$headersolid"_"$report"$$.PDF"
MrtFile=dentillmn001.jasper
LstFile=$headersolid"_"$report"$$.lst"
#******************************************************************************************
#   SQL Execution
#******************************************************************************************
    exebatch spbx4001 custom.FIN_DENTILL_PACK.FIN_DENTILL_PROC  "$solid" "$telId" "$headersolid" "$CRNCY_CODE" "$BANK_ID" > $LstFile
	if test ! -s "$LstFile"
    then
...
#******************************************************************************
    ...
    export FINRPT_CLIENT_SOCKARRAY_SIZE
      #mrbx4004 $B2K_SESSION_ID $MrtFile $LstFile $RptFile1 -f 1NNNN1
