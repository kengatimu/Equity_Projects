/
exec custom.booked_int_daily_insert;
/
commit;
/
...
where item='BOOKED INTEREST' and BOD_DATE >=TO_CHAR((TO_DATE(sysdate-1,'DD-MM-RRRR')),'DD-MON-RRRR')
GROUP BY  COUNTRY
union all
...
and BOD_DATE >=TO_CHAR((TO_DATE(sysdate-1,'DD-MM-RRRR')),'DD-MON-RRRR')
...
as Booked
from CUSTOM.BOOKED_SUS_INT_TABLE bs
where item='SUSPENDED INTEREST' and BOD_DATE >=TO_CHAR((TO_DATE(sysdate-1,'DD-MM-RRRR')),'DD-MON-RRRR') 
GROUP BY  COUNTRY
union all
select (select decode (Bank_id,'54','KE','11','SS','55','TZ','50','RW','56', 'UG','99','FS') from tbaadm.gct where del_flg='N'and gct.bank_id=bs.COUNTRY) ||':'||trim((to_char(SUM(AMOUNT_KES),'999,999,999,999,999.99')))
as Booked
...
and PART_TRAN_TYPE='C'  and tran_date>=TO_CHAR((TO_DATE(sysdate-1,'DD-MM-RRRR')),'DD-MON-RRRR')
