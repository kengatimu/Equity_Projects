...
Rem : Date              28-09-2022
Rem : Description       Purge data from agddt to agddt_hist_tbl  ( move all deleted records to history table)
...
WHERE
        bank_id = '&1' 
and free_text1 in (select cif_id from tbaadm.gam where acid in
(select acid from tbaadm.fcftt where bank_id='&1' and fcftt.free_code_18='AGENT-FLOAT')); 
BEGIN
    commit_flg := 'N';
