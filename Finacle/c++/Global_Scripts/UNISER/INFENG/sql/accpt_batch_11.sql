...
spool ACPART_INSERT
DECLARE
        bankid           tbaadm.acpart.bank_id%type :='11';
       --v_acct_id          tbaadm.gam.foracid%type :='';
        ...
        from tbaadm.acpart where B2K_TYPE='ACCNT' and pstd_flg='Y' and bank_id=11 and (b2k_id, bank_id) in (
        select acid, bank_id from tbaadm.gam where 
    --foracid=v_acct_id
    schm_type='LAA' and clr_bal_amt=0 and bank_id='11') /* Changes for Setting as batch job. */
