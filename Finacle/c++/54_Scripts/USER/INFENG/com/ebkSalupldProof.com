...
if [ "${chargecode}" = "EMP_S" ]
then
	bauu9151 ebkSalProof_upload_SAL.sql ${sqlInput}
else
	if [ "${chargecode}" = "EMP_F" ]
	then
		bauu9151 ebkSalProof_upload_SAL.sql ${sqlInput}
...
exit 0
#failed charge collection accounts Gichiri
failedchrgfile=${ONS_LOG_DIR}/${userid}/errorchracct.log
test -f ${failedchrgfile}
if [ $? -eq 0 ]
then
	exebatch babx4040 ${B2K_SESSION_ID} ${failedchrgfile}  "CSALBPP FAILED CHARGE ACCT" "MANAGER" "1" "N"
