...
reportFileName="egokb-${toDate}-${toTime}"
lstFileName="egokb${$}.lst"
ripFileName="egokb${$}.rip"
mrtFileName="egokb.mrt"
datFileName="egokb${$}.dat"
userid=`pwd|awk -F "/" '{print $NF}'`
rptpath="/finreports/FIN_REPORTS_DIR/${userid}/"
rptpath2="/finreports/FIN_REPORTS_DIR/VER50/"
...
#exebatch bauu9151 egokb.sql "${fromDate}" "${toDate}" "${field_3}" 
exebatch bauu9151 egokb.sql "${toDate}" "${bankId}" "${lstFileName}" "${fromDate}"
if [ $? -ne 0 ]
then
	echo "SQL procedure execution for egokb.sql failed !"
	exit 1
fi
...
mv ${rptpath}$reportFileName.rpt ${rptpath2}$reportFileName.txt
