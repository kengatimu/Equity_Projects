...
if ('&3' = '50') then
        dbms_output.put_line(i.BANK_CODE||'1'||'~'||'~'||i.BANK_CODE||SUBSTR(i.BRANCH_CODE,2,'3')||'~'||SUBSTR(FORACID1,4)||'~'||i.gamacctcrncycode||i.gamacctcrncycodebook||lpad(i.NUM_OF_CHB,2,'0')||lpad(i.BEGIN_CHQ_NUM,8,'0')||'~'||'BANKERS CHEQUE'||'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'||rpad(i.BR_NAME,30,'~')||rpad(i.BR_ADDR_1,30,'~')||rpad(FORACID1,15,'~')||'        ');
end if;

if ('&3' = '56') then
	dbms_output.put_line(i.BANK_CODE||'1'||'~'||i.BANK_CODE||substr(FORACID1,3,2)||'47'||lpad(substr(FORACID1,4,13),10)||lpad(CHQDIGIT,2,'0')||'13'||'17'||lpad(i.NUM_OF_CHB,2,'0')||lpad(i.BEGIN_CHQ_NUM,6,'0')||rpad(i.BR_NAME,80,'~')||rpad(i.BR_NAME,30,'~')||rpad(i.BR_ADDR_1,30,'~')||rpad(FORACID1,15,'~'));
end if;
