--DROP PACKAGE CUSTOM.GETCHQDETAILDWNLD_54;

CREATE OR REPLACE PACKAGE CUSTOM.Getchqdetaildwnld_54 AS
PROCEDURE Getchqdetaildwnld( inp_str IN VARCHAR2,
            out_retCode OUT NUMBER,
            out_desc OUT VARCHAR2);
END Getchqdetaildwnld_54;
/


GRANT EXECUTE, DEBUG ON CUSTOM.GETCHQDETAILDWNLD_54 TO TBAADM;

GRANT EXECUTE, DEBUG ON CUSTOM.GETCHQDETAILDWNLD_54 TO TBAGEN;

GRANT EXECUTE, DEBUG ON CUSTOM.GETCHQDETAILDWNLD_54 TO TBAUTIL;


DROP PACKAGE BODY CUSTOM.GETCHQDETAILDWNLD_54;

CREATE OR REPLACE PACKAGE BODY CUSTOM.Getchqdetaildwnld_54 AS
...
--    out_desc := '68'||'|'||BRCODE||'|'||FORACID1||'|'||CUSTNAME||'|'||'SB'||CHQLVS||'|'||'KES'||'|'||vCHQBOOKS||'|'||vCHSSR||'|'||vENDNO1||'|'||vREQDATE ;
