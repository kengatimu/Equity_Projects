...
cd /finacle/EQDR/Fin10218/APP/Finacle/FC/app/CDCI_LOGS/CDCI${bankId}
BatId=${1}
bankId=`echo $FIN_BANK_ID`
if [ "$bankId" == "54" ]
then
...
#}
        ...
        echo "select rpad(NVL(ACCT_NUMBER,' '),16,' ')||rpad((select acct_crncy_code from tbaadm.gam where bank_id='${bankId}' and foracid=ACCT_NUMBER),3,' ')||rpad(NVL(nvl((select sol_id from tbaadm.gam where bank_id='${bankId}' and foracid=ACCT_NUMBER),'000'),' '),8,' ')|| rpad(NVL(PART_TRAN_TYPE,' '),1,' ')|| lpad(NVL(abs((TRAN_AMT)),0),17,' ')|| rpad(NVL(TRAN_PARTICULAR,' '),30,' ')|| rpad(NVL(' ',' '),5,' ')||rpad(NVL(free_text1,' '),20,' ')|| rpad(NVL(' ',' '),5,' ')|| rpad(NVL(' ',' '),10,' ')||rpad(NVL(' ',' '),6,' ')|| rpad(NVL(' ',' '),16,' ')|| " >> ${BatId}_T.sql
        echo " rpad(NVL(' ',' '),1,' ')||(case when (select acct_crncy_code from tbaadm.gam where bank_id='${bankId}' and foracid=ACCT_NUMBER) <> ref_crncy_code then lpad(NVL(abs((TRAN_AMT)),0),17,' ') else rpad(NVL(' ',' '),17,' ') end) ||(case when (select acct_crncy_code from tbaadm.gam where bank_id='${bankId}' and foracid=ACCT_NUMBER) <> ref_crncy_code then rpad(REF_CRNCY_CODE,3,' ') else rpad(NVL(' ',' '),3,' ') end) ||(case when (select acct_crncy_code from tbaadm.gam where bank_id='${bankId}' and foracid=ACCT_NUMBER) <> ref_crncy_code then  rpad(NVL('MEAN',' '),5,' ') else rpad(NVL(' ',' '),5,' ')end) || " >> ${BatId}_T.sql
        echo "(case when (select acct_crncy_code from tbaadm.gam where bank_id='${bankId}' and foracid=ACCT_NUMBER) <> ref_crncy_code then nvl((select rpad(NVL(VAR_CRNCY_UNITS,''),15,' ') from tbaadm.rtl where bank_id='${bankId}' and fxd_crncy_code=ref_crncy_code and var_crncy_code=(select acct_crncy_code from tbaadm.gam where bank_id='${bankId}' and foracid=ACCT_NUMBER) and ratecode='MEAN' and entity_cre_flg='Y' and del_flg='N' and RTLIST_NUM in (select max(RTLIST_NUM) from tbaadm.rtl where bank_id='${bankId}' and fxd_crncy_code=ref_crncy_code and var_crncy_code=(select acct_crncy_code from tbaadm.gam where bank_id='${bankId}' and foracid=ACCT_NUMBER) and ratecode='MEAN' and entity_cre_flg='Y' and del_flg='N')),"  >> ${BatId}_T.sql
        echo "(select rpad(NVL(VAR_CRNCY_UNITS,''),15,' ') from tbaadm.rtl where bank_id='${bankId}' and fxd_crncy_code=(select acct_crncy_code from tbaadm.gam where bank_id='${bankId}' and foracid=ACCT_NUMBER) and var_crncy_code=ref_crncy_code and ratecode='MEAN' and entity_cre_flg='Y' and del_flg='N' and RTLIST_NUM in (select max(RTLIST_NUM) from tbaadm.rtl where bank_id='${bankId}' and fxd_crncy_code=(select acct_crncy_code from tbaadm.gam where bank_id='${bankId}' and foracid=ACCT_NUMBER) and var_crncy_code=ref_crncy_code and ratecode='MEAN' and entity_cre_flg='Y' and del_flg='N'))) else rpad(NVL('',' '),15,' ') end) || rpad(to_char(sysdate,'DD-MM-YYYY'),10,' ')|| " >> ${BatId}_T.sql
        echo " rpad(NVL(' ',' '),10,' ')|| rpad(NVL(' ',' '),5,' ')|| rpad(NVL(' ',' '),6,' ')|| rpad(NVL(' ',' '),6,' ')|| rpad(NVL(' ',' '),2,' ')|| rpad(NVL(' ',' '),1,' ')|| rpad(NVL(' ',' '),12,' ')|| rpad(NVL(' ',' '),10,' ')|| rpad(NVL(' ',' '),20,' ')|| rpad(NVL(' ',' '),5,' ')|| rpad(NVL(' ',' '),30,' ')|| rpad(NVL(' ',' '),40,' ')|| rpad(NVL(' ',' '),40,' ')|| rpad(NVL(' ',' '),40,' ')|| rpad(NVL(' ',' '),40,' ')|| rpad(NVL(' ',' '),40,' ')|| rpad(NVL(' ',' '),17,' ')|| rpad(NVL(' ',' '),17,' ')|| rpad(NVL(' ',' '),17,' ')|| rpad(NVL(' ',' '),17,' ')|| rpad(NVL(' ',' '),17,' ')|| rpad(NVL(TRAN_PARTICULAR,' '),30,' ')|| rpad(NVL(' ',' '),16,' ')|| rpad(NVL(' ',' '),12,' ')|| rpad(NVL(' ',' '),10,' ')|| rpad(NVL(' ',' '),10,' ')|| rpad(NVL(' ',' '),9,' ')|| rpad(NVL(' ',' '),4,' ')|| rpad(NVL(' ',' '),256,' ')|| rpad(NVL(' ',' '),16,' ')|| rpad(NVL(' ',' '),5,' ')|| rpad(NVL(' ',' '),5,' ')|| rpad(NVL(' ',' '),5,' ')|| rpad(NVL(' ',' '),50,' ') from custom.cust_merchant_tran_tbl where bank_id='${bankId}' and TRAN_TYPE='T' and UPL_BATCH_ID='${BatId}' and PART_TRAN_TYPE in ('D','C') and PROCESS_STATUS='U' and entity_cre_flg='Y' and del_flg='N' and PROCESS_TRAN_ID is null and process_date is null;" >> ${BatId}_T.sql
        ...
        cp -f ${BatId}.DAT /finreports/FILE_UPLOAD/CDCI${bankId}/
        sleep 3
        drmsg="P"
...
echo "a is " $tid > log.txt
        ...
        then
                ...
                echo "update custom.cust_merchant_tran_tbl set PROCESS_TRAN_ID='${tid}',process_date=sysdate,process_desc='TRAN',lchg_user_id='${usr}',lchg_time=sysdate where bank_id='${bankId}' and upl_batch_id='${BatId}' and rcre_user_id='${usr}' and PROCESS_TRAN_ID is null;" >>${BatId}_status.sql
