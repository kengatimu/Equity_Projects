...
optDirectory=opt/agent/_work/53/a

#FI path and variable definitions
hpFIRootDirectory=/equity_be/EQPRODFI/BackEnd/Finacle/FC/app
hpFIScriptDirectory=/equity_be/EQPRODFI/BackEnd/Finacle/FC/app/prod_cust
...
FINewFiles="$TempFIScriptDir"/"$optDirectory"/newFiles
FIDeletedFiles="$TempFIScriptDir"/"$optDirectory"/deletedFiles
FIModifiedFiles="$TempFIScriptDir"/"$optDirectory"/modifiedFiles
FIFinacleFileType="$TempFIScriptDir"/"$optDirectory"/finacleFileType
...
cleanup() {
	...
	rm -rvf deploy-fi-scripts.sh || true
	rm -rvf check-modified-files.sh || true
	rm -rvf finacle-FI-scripts.tar.gz || true
}
