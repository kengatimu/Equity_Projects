...
optDirectory=opt/agent/_work/54/a

#UNISER path and variable definitions
hpUNISERRootDirectory=/equity_be/EQPRODUNISER/BackEnd/Finacle/FC/app
hpUNISERScriptDirectory=/equity_be/EQPRODUNISER/BackEnd/Finacle/FC/app/prod_cust
...
UNISERNewFiles="$TempUNISERScriptDir"/"$optDirectory"/newFiles
UNISERDeletedFiles="$TempUNISERScriptDir"/"$optDirectory"/deletedFiles
UNISERModifiedFiles="$TempUNISERScriptDir"/"$optDirectory"/modifiedFiles
UNISERFinacleFileType="$TempUNISERScriptDir"/"$optDirectory"/finacleFileType
...
cleanup() {
	...
	rm -rvf deploy-uniser-scripts.sh || true
	rm -rvf check-modified-files.sh || true
	rm -rvf finacle-UNISER-scripts.tar.gz || true
}
...
then
    echo "FiNACLE FI FILES FOUND IN:  "$UNISERFinacleFileType"/finacleFileType.txt"
