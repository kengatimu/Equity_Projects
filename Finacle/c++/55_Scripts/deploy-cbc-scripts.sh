...
optDirectory=opt/agent/_work/56/a

#CBC path and variable definitions
hpCBCRootDirectory=/equity_be/EQPRODCBC/BackEnd
hpCBCScriptDirectory=/equity_be/EQPRODCBC/BackEnd/PROD_CBC
...
CBCNewFiles="$TempCBCScriptDir"/"$optDirectory"/newFiles
CBCDeletedFiles="$TempCBCScriptDir"/"$optDirectory"/deletedFiles
CBCModifiedFiles="$TempCBCScriptDir"/"$optDirectory"/modifiedFiles
CBCFinacleFileType="$TempCBCScriptDir"/"$optDirectory"/finacleFileType
...
cleanup() {
	...
	rm -rvf deploy-cbc-scripts.sh || true
	rm -rvf check-modified-files.sh || true
	rm -rvf finacle-CBC-scripts.tar.gz || true
}
...
#check if any CBC files exists, if not, exit the program
