...
commit;
/
exec CUSTOM.SMS_PROC_TRANSALERTS_3;
/
commit;
/
exec CUSTOM.SMS_INSERT_PROC_TRANSALERTS_3;
/
commit;
/
exec CUSTOM.DUTY_SERVICES_UPDATE;
/
commit;
/
update crmuser.accounts_mod set RECORDSTATUS ='F' ,CURRENTSTEP ='2'  where recordstatus='D' and orgkey in (select cif_id from custom.bulk_cif where entity_cre_flg ='F') AND RECORDSTATUS ='D';
/
commit;
/
