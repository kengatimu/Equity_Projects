...
INSERT INTO custom.c_oflow_vat_tbl
   SELECT m.foracid,
          (SELECT acid
             FROM tbaadm.gam
            WHERE gam.foracid = m.foracid AND gam.bank_id = m.bank_id) acid,
          (SELECT sol_id
             FROM tbaadm.gam
            WHERE gam.foracid = m.foracid AND gam.bank_id = m.bank_id) sol_id,
          (SELECT acct_crncy_code
             FROM tbaadm.gam
            WHERE gam.foracid = m.foracid
              AND gam.bank_id = m.bank_id) acct_crncy_code,
          (SELECT oflow_amt
             FROM tbaadm.lam
            WHERE lam.acid =
                     ((SELECT acid
                         FROM tbaadm.gam
                        WHERE gam.foracid = m.foracid
                          AND gam.bank_id = m.bank_id))
              AND lam.bank_id = m.bank_id) oflow_amt,
          (SELECT MIN (next_int_dmd_date)
             FROM tbaadm.lrs
            WHERE lrs.acid =
                     ((SELECT acid
                         FROM tbaadm.gam
                        WHERE gam.foracid = m.foracid
                          AND gam.bank_id = m.bank_id))
              AND lrs.bank_id = m.bank_id
              AND lrs.entity_cre_flg = 'Y'
              AND del_flg = 'N') next_int_dmd_date,
          SYSDATE lchg_time, SYSDATE rcre_time,
          (SELECT db_stat_date - 1
             FROM tbaadm.gct
            WHERE bank_id = m.bank_id) last_run_date,
             'CELPP' rcre_user_id,
          'CELPP' lchg_user_id,
          m.bank_id bank_id
     FROM custom.equ_upload_table m
    WHERE valid = 'Y'
	and m.foracid not in (select foracid from custom.c_oflow_vat_tbl where bank_id = m.bank_id);
