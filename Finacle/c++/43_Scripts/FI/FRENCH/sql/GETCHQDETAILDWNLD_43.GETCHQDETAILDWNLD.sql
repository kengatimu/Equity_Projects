CREATE OR REPLACE PACKAGE BODY CUSTOM.Getchqdetaildwnld_43 AS
...
BEGIN
    ...
    IF Get_Account%FOUND
      THEN
         NUMBEROFREC := Get_Account%ROWCOUNT;
      END IF;
...
--    out_desc := '|68'||'|'||BRCODE||'|'||FORACID1||'|'||CUSTNAME||'|'||SCHMDESC||'|'||ACCTCRNCY||'|'||vCHQBOOKS||'|'||CHQLVS||'|'||vCHSSR||'|'||vENDNO1||'|'||vREQDATE||'|' ;
    IF (NUMBEROFREC = 1)
      THEN
      out_desc := HEADERFORFILE || chr(10) || hardcode1||' '||hardcode2||RPAD(solid||''||SUBSTR(FORACID1,4),13,' ')||CHQDIGIT||vTrancode||trim(BKSTYLE)||vCHQBOOKS||RPAD(vCHSSR,6,' ')||RPAD(CUSTNAME,80,' ')||RPAD(BRNAME,30,' ')||RPAD(BRADDRESS,30,' ')||RPAD(FORACID1,15,' ');
      ORDERREC := FOOTERCNT;
     ELSE
