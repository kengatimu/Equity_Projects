...
public class ReferenceNumberRequestController {
    private static final Logger log = LoggerFactory.getLogger(ReferenceNumberRequestController.class);
    ...
    @Autowired
    public ReferenceNumberRequestController(@Value("${signature.validation.isInternal}") boolean isInternal,
    ...
    @PostMapping(value = "/referenceNumber",
    ...
    public ResponseEntity<?> handleOmniRequest(@RequestBody String requestXml, WebRequest webRequest) throws CustomException {
        String type = "moneygram-receive-reference-number-request";
        String rrn = "";
        try {
