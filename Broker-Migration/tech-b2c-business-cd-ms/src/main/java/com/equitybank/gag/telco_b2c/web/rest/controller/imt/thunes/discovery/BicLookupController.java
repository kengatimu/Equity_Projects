...
public class BicLookupController {
    private static final Logger log = LoggerFactory.getLogger(BicLookupController.class);

    private final Gson gson;
    private final EnabledServicesChecker enabledServicesChecker;
    private final RequestExecutorService requestExecutorService;

    public BicLookupController(Gson gson,
    ...
    @PostMapping(value = "/lookups/bic", consumes = {MediaType.APPLICATION_JSON_VALUE}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<?> handleChannelRequest(@Valid @RequestBody BicLookup bicLookup, BindingResult bindingResult, WebRequest webRequest) throws CustomException {
        String type = "thunes-bic-lookup";
        ...
        try {
            ...
            log.info("{}: Received {} Request From Channel: \n{}", rrn, type, gson.toJson(bicLookup));
            ChannelResponse channelResponse = requestExecutorService.executeThunesBicLookup(rrn, bicLookup, bindingResult, type);
