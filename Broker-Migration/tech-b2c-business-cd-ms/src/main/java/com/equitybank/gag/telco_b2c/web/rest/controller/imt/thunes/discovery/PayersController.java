...
public class PayersController {
    private static final Logger log = LoggerFactory.getLogger(PayersController.class);

    private final Gson gson;
    private final EnabledServicesChecker enabledServicesChecker;
    private final RequestExecutorService requestExecutorService;

    public PayersController(Gson gson,
    ...
    @PostMapping(value = "/payers", consumes = {MediaType.APPLICATION_JSON_VALUE}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<?> handleChannelRequest(@Valid @RequestBody ThunesPayers thunesPayers, BindingResult bindingResult, WebRequest webRequest) throws CustomException {
        String type = "thunes-get-payers";
        ...
        try {
            ...
            log.info("{}: Received {} Request From Channel: \n{}", rrn, type, gson.toJson(thunesPayers));
            ChannelResponse channelResponse = requestExecutorService.executeThunesGetPayers(rrn, thunesPayers, bindingResult, type);
