...
public class CreditPartyVerifyController {
    private static final Logger log = LoggerFactory.getLogger(CreditPartyVerifyController.class);

    private final Gson gson;
    private final EnabledServicesChecker enabledServicesChecker;
    private final RequestExecutorService requestExecutorService;

    public CreditPartyVerifyController(Gson gson,
    ...
    @PostMapping(value = "/credit-party-verification", consumes = {MediaType.APPLICATION_JSON_VALUE}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<?> handleChannelRequest(@Valid @RequestBody CreditPartyVerify creditPartyVerify, BindingResult bindingResult, WebRequest webRequest) throws CustomException {
        String type = "thunes-credit-party-verify";
        ...
        try {
            ...
            log.info("{}: Received {} Request From Channel: \n{}", rrn, type, gson.toJson(creditPartyVerify));
            ChannelResponse channelResponse = requestExecutorService.executeThunesCreditPartyVerify(rrn, creditPartyVerify, bindingResult, type);
