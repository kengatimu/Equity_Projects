...
public class ReceiveCommitTransactionController {
    private static final Logger log = LoggerFactory.getLogger(ReceiveCommitTransactionController.class);
    ...
    @Autowired
    public ReceiveCommitTransactionController(@Value("${signature.validation.isInternal}") boolean isInternal,
    ...
    public ResponseEntity<?> handleOmniRequest(@RequestBody String requestXml, WebRequest webRequest) throws CustomException {
        String type = "moneygram-receive-commit-transaction";
        String rrn = "";
        try {
