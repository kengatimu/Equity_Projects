...
public class PayerDetailsController {
    private static final Logger log = LoggerFactory.getLogger(PayerDetailsController.class);

    private final Gson gson;
    private final EnabledServicesChecker enabledServicesChecker;
    private final RequestExecutorService requestExecutorService;

    public PayerDetailsController(Gson gson,
    ...
    @PostMapping(value = "/payers/details", consumes = {MediaType.APPLICATION_JSON_VALUE}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<?> handleChannelRequest(@Valid @RequestBody PayerDetails payerDetails, BindingResult bindingResult, WebRequest webRequest) throws CustomException {
        String type = "thunes-payer_details";
        ...
        try {
            ...
            log.info("{}: Received {} Request From Channel: \n{}", rrn, type, gson.toJson(payerDetails));
            ChannelResponse channelResponse = requestExecutorService.executeThunesPayerDetails(rrn, payerDetails, bindingResult, type);
