...
public class ReceiveReversalRequestController {
    private static final Logger log = LoggerFactory.getLogger(ReceiveReversalRequestController.class);
    ...
    @Autowired
    public ReceiveReversalRequestController(@Value("${signature.validation.isInternal}") boolean isInternal,
    ...
    @PostMapping(value = "/receiveReversal",
    ...
    public ResponseEntity<?> handleOmniRequest(@RequestBody String requestXml, WebRequest webRequest) throws CustomException {
        String type = "moneygram-receive-reversal";
        String rrn = "";
        try {
