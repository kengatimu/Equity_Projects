package com.equitybank.gag.telco_b2c.web.rest.controller.imt.thunes.discovery;
...
public class PayerRatesController {
    private static final Logger log = LoggerFactory.getLogger(PayerRatesController.class);

    private final Gson gson;
    private final EnabledServicesChecker enabledServicesChecker;
    private final RequestExecutorService requestExecutorService;

    public PayerRatesController(Gson gson,
    ...
    @PostMapping(value = "/payers/rates", consumes = {MediaType.APPLICATION_JSON_VALUE}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<?> handleChannelRequest(@Valid @RequestBody PayerRates payerRates, BindingResult bindingResult, WebRequest webRequest) throws CustomException {
        String type = "thunes-payer_rates";
        ...
        try {
            ...
            log.info("{}: Received {} Request From Channel: \n{}", rrn, type, gson.toJson(payerRates));
            ChannelResponse channelResponse = requestExecutorService.executeThunesPayerRates(rrn, payerRates, bindingResult, type);
