...
public class AirtimeTopupNamecheckController {
    private static final Logger log = LoggerFactory.getLogger(AirtimeTopupNamecheckController.class);
    ...
    @Autowired
    public AirtimeTopupNamecheckController(@Value("${signature.validation.isInternal}") boolean isInternal,
    ...
    @PostMapping(value = "/airtimetopup/namecheck",
    ...
    public ResponseEntity<?> handleChannelRequest(@RequestBody String requestXml, WebRequest webRequest) throws CustomException {
        String type = "airtime-topup-namecheck";
        String rrn = "";
        try {
            // Check if service is enabled
            enabledServicesChecker.checkServiceStatus(enabledServicesChecker.isNameCheckAirtimeTopupEnabled(), type +" service");
