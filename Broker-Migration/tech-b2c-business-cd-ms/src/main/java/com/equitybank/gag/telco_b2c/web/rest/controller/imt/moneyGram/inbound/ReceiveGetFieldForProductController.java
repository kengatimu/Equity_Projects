...
public class ReceiveGetFieldForProductController {
    private static final Logger log = LoggerFactory.getLogger(ReceiveGetFieldForProductController.class);
    ...
    @Autowired
    public ReceiveGetFieldForProductController(@Value("${signature.validation.isInternal}") boolean isInternal,
    ...
    public ResponseEntity<?> handleOmniRequest(@RequestBody String requestXml, WebRequest webRequest) throws CustomException {
        String type = "moneygram-receive-get-field-for-product";
        String rrn = "";
        try {
