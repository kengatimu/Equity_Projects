package com.equitybank.gag.telco_b2c.web.rest.controller.imt.thunes.quote;
...
public class CreateQuoteController {
    private static final Logger log = LoggerFactory.getLogger(CreateQuoteController.class);

    private final Gson gson;
    private final EnabledServicesChecker enabledServicesChecker;
    private final RequestExecutorService requestExecutorService;

    public CreateQuoteController(Gson gson,
    ...
    @PostMapping(value = "/quotation", consumes = {MediaType.APPLICATION_JSON_VALUE}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<?> handleChannelRequest(@Valid @RequestBody CreateQuoteDto createQuoteDto, BindingResult bindingResult, WebRequest webRequest) throws CustomException {
        String type = "thunes-create-quote";
        ...
        try {
            ...
            log.info("{}: Received {} Request From Channel: \n{}", rrn, type, gson.toJson(createQuoteDto));
            ChannelResponse channelResponse = requestExecutorService.executeThunesCreateQuotation(rrn, createQuoteDto, bindingResult, type);
