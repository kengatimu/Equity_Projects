...
@Service("imtAmendTransactionStrategy")
public class ImtAmendTransactionStrategyService implements XmlTypeProcessingStrategyService {
    private static final Logger log = LoggerFactory.getLogger(ImtAmendTransactionStrategyService.class);
    ...
    public ImtAmendTransactionStrategyService(XmlFieldsValidatorService xmlFieldsValidatorService,
    ...
    public String process(boolean isInternal, String requestXml, Map<String, String> xmlTagsMap, String rrn, String type) throws CustomException, SignatureException {
        ...
        // Extract bank ID from request map
        String bankId = xmlTagsMap.get("bankId");

        // Load existing DB record using RRN
        TransactionDetails transactionDetails = getSavedDbRecord(rrn, type);

        // Call external service and retrieve XML response
        ...
        // Update database with any incoming changes
        updateTransactionRecordWithRequestInputs(rrn, xmlTagsMap, type);

        // Return parsed and updated response to channel
